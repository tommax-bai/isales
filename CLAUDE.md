...
- **Option menus end with a 「说人话」 summary.** When presenting the user a numbered choice (1/2/3, A/B/C, "下一步选哪个" 之类), after the detailed breakdown ALWAYS append a closing "说人话" section — 1-3 sentences max, everyday vocabulary, no technical jargon, restating the choice in plain terms and naming the recommendation. The user reads the 说人话 first to orient, then dives into the detail only if needed. Applies whether the options are technical (next-step picks), process (workflow / commit boundary decisions), or scope (what to include / cut). Skip it only when the choice is genuinely trivial (yes/no with no nuance).
