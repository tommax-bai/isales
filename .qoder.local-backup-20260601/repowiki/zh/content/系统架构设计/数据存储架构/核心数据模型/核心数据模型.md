...
- pipeline_trace：AI 三层管线的候选、裁判、润色追踪。
...
### 表：role_config
- 归属服务：api
- 关键字段：campaign_id、kind(role/judge/polish)、model、current_prompt_version_id、temperature、top_p、ext_params(JSONB)、enabled
...
### 表：prompt_version
- 归属服务：api
- 关键字段：scope_type(role/judge/polish)、scope_id、content、created_at、created_by、is_active
...
### 表：pipeline_trace
- 归属服务：engine
- 关键字段：call_record_id、turn_id、ts_start、ts_end、user_input、role_candidates(JSONB)、judge_results(JSONB)、polish_input(JSONB)、polish_output、polish_duration_ms、polish_role_config_id、polish_prompt_version_id、final_selected_candidate_index
- 业务含义：AI 三层管线的候选、裁判、润色追踪，用于调试与优化。
...
### 表：call_record
- 归属服务：engine
- 关键字段：lead_id、campaign_id、caller_id、status、started_at、ended_at、duration、transcript(JSONB)、recording_url、transfer_status、transfer_reason、wrap_up_started_at、prompt_versions(JSONB)
...
### 表：call_summary
- 归属服务：worker
- 关键字段：call_record_id、summary_text、extracted_fields(JSONB)、goal_achieved、goal_type
...
## 结论
本文基于 openspec 的数据模型规范，系统化梳理了 iSales 的 12 张核心表，明确了归属服务、字段语义、表间关系、JSONB 结构与使用方式、典型查询与权限边界，并提供了性能与故障排查建议。建议在实现与运维中严格遵循“模型与迁移由 isales-common 统一管理”的原则，确保跨服务一致性与演进可控。
...
- pipeline_trace.role_candidates/judge_results/polish_input(JSONB)：结构化 JSON，包含候选、裁判结果、润色输入输出。
