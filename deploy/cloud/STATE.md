...
**Last updated**: 2026-06-21 11:30 CST — **域名预接线 + 发现 aidcp co-tenant（纯 nginx `server_name`，无 isales 代码/DB 改动）**。用户注册域名 `tommax.cc`，规划 `isales.tommax.cc`→iSales、`aidcp.tommax.cc`→AIDCP。**关键发现**：ECS 上仍只有 **1 个 nginx**（master `pid 720` + 2 worker），但除 isales 外还托管一个 **co-tenant 项目 AIDCP**——`aidcp-cloud.service`（active，"AIDCP Cloud - Soul Orchestrator + Qwen + WebSocket Server"，node 监听 `127.0.0.1:8090`），nginx drop-in `/etc/nginx/conf.d/aidcp-console.conf` 原 `listen 8088` 反代 → `:8090`，root `/opt/aidcp/console`（uid 501，**非 isales 管理、不在本仓库**），2026-06-20 才上线（conf mtime 06-20 18:37）。它跟 iSales 共用这台 ECS 的 CPU/内存/带宽——engine 实时性抖动时是嫌疑邻居。**改动（附加式，备份+`nginx -t`+失败自动回滚）**：①`isales.conf` 加 `server_name isales.tommax.cc;`（仍保 `listen 80 default_server` 当兜底 catch-all）；②`aidcp-console.conf` 加 `listen 80; server_name aidcp.tommax.cc;`（保留 8088 向后兼容）。备份 `/etc/nginx/conf.d/{isales,aidcp-console}.conf.bak.20260621`。Host 头实测：`isales.tommax.cc`→「iSales 智能外呼」200 / `aidcp.tommax.cc`→「AIDCP 运营管理后台」200 / `:8088` 仍 200 / 未知 Host 兜底 isales 200。**⚠️ 域名现在还打不开**：`tommax.cc` 未 ICP 备案，国内 ECS 用域名走 :80 会被阿里云网络层拦——须先加 DNS A 记录（`isales`/`aidcp.tommax.cc` → `121.89.85.150`）**且**完成 ICP 备案，期间仍用 IP `http://121.89.85.150/`（不受影响）。**HTTPS/443 未配**（用户选先只配 HTTP，留待备案后）。**⚠️ 漂移风险**：`aidcp-console.conf` 是他方项目配置、不在本仓库，其自有部署若重写该文件会丢这次 `server_name` 增量。仓库侧 `isales-web/deploy/nginx.conf` `server_name` 同步为 `isales.tommax.cc`（注：`deploy/cloud/nginx/isales.conf` 是 443+gRPC 理想态、非生产跑的那份）。**回滚**：`mv /etc/nginx/conf.d/{isales,aidcp-console}.conf.bak.20260621 → .conf` + `nginx -s reload`。

---

Prior: 2026-06-20 09:57 CST — **engine-interruption-cover-thinking-window deployed（engine 单文件×2 scp + 重启 engine；纯 engine，无 common/alembic/api/web）**。修「客户回完第一句、短暂停顿、AI 还没出声时又说第二句 → AI 仍先答第一句、对话永久慢一拍（错乱/滞后）」。根因=barge-in 判定硬绑「正在播音」：`_partial_monitor` 死区守卫 `current_speaking_task is None` 在 THINKING 窗口（已收 A 的 ASR final、main LLM 生成中、首个 TTS 音频未释放）把第二句 B 的 partial 当状态更新丢弃，B 的 final 排队等下一轮 → 慢一拍。**修法（复用现有打断机制、只扩覆盖范围、无新机制）**：①新增 `session.in_processing_turn`（与 `current_speaking_task`/`in_wrap_up` 同类状态位）；②`_partial_monitor` 守卫放宽为 `(not in_processing_turn and current_speaking_task is None) or interruption_signaled`——THINKING 窗口也走**同一棵** `evaluate_partial` 规则树（阈值/白名单逐字节不变），LISTENING 仍不触发；③`_run_gated_turn` 把「spawn 候选+门控 await」包进 try/finally 持 `in_processing_turn`（[entry→gate-resolve]），门控返回后 check#1 若 `interruption_signaled` → 取消 eager 候选、`consecutive_interruption_count+=1`、落 `first_audio_ms=None` trace、return CONTINUE，主循环既有 coalesce 把 A+B 合并经 **main 管线（裁判门完整）** 重答；A 已在 dialog_history 不丢；**不写 `interrupt_remaining_text`→ 永不 restructure**（思考窗口一字未播无半句可续）。[gate→首音频] 尾窗由现有播音 barge-in 覆盖（故未改 `_play_streaming`）。**部署**：①**preflight md5 实证盒子 `run_loop.py`(`6720f227`)+`call_session.py`(`b6545e04`) == git base `59b2fae`**（无漂移、run_loop 是并行热点文件不 clobber）；②备份 `/opt/isales/backups/{run_loop,call_session}.py.bak-20260620-095725-thinking-window`；③scp 2 文件 → `/opt/isales/current/isales-engine/isales_engine/` + chown isales:isales（post-scp md5 `508f6c13`/`da24b920` == git HEAD `5709d2e`）；④`systemctl restart isales-engine`（active）。**探针**：`cloud_edge_grpc_server_started`+`credentials_loaded count=6`+`isales_engine_started` 无 traceback；`in_processing_turn` 在 run_loop(3)/call_session(1)；**他方改动仍在**（`set_barge_in_fadeout`=1 / `main_llm_prewarm`=7，未 clobber 并行 work）。commit engine `5709d2e` / meta `ded1f69`(openspec propose+apply)+本条 STATE。本机测试 engine **467 passed / 22 skipped / 1 pre-existing fail**（`test_gate_fails_open_to_main_on_referee_timeout`，改前干净树同红、无关）；+4 新测（`test_realtime_interruption.py`：THINKING 触发 / 思考窗白名单忽略 / LISTENING 不触发 / 端到端 abort）。**⚠️ 真机验收 pending → change 仍 active**：需真拨测复现「客户说 A → 停顿（AI 没出声）→ 说 B」确认 AI 合并 A+B 一次答清（不再慢一拍）、语气词在思考窗口仍不误打断、播音中 barge-in→重组不回归 → 过了才 archive。**回滚**：scp 回 `/opt/isales/backups/{run_loop,call_session}.py.bak-20260620-095725-thinking-window` + 重启 engine（纯单文件×2，无 DB/无 common/无 alembic）。

---

Prior: 2026-06-17 21:15 CST — **engine-barge-in-fade-out (Slice 1+2) deployed（common 0.8.22→0.8.23 + alembic `f0a1b2c3d4e5` + engine 5 文件 + api `schemas.py` 镜像 + web 重构建；engine/api/scheduler/worker 全重启）**。打断收声从硬切改渐隐淡出：barge-in 取消时不再把 playout 队列清空到静音（帧边界振幅阶跃=爆音 click + 比真人快），而是把队首 `ceil(fadeout_ms/10)` 帧经 `ambient.apply_fadeout`（per-sample 线性增益斜坡到 0，纯 `array('h')` 无 audioop）重新入队、丢其余，让人声渐弱收住。**出向路径统一**（D3）：mixing pump 改常驻（`start_outbound_pump` idempotent，configure_ambient/audio_out 均起），淡出只写 `_flush_playout` 一处即覆盖 ambient 开/关两路径；旧无缓冲直推路径连同死代码 `next_timestamp_ms`/`outbound_frame_ms` 删除；`_outbound_loop` 改每帧读 `ambient_reader`/`gain`（修 pump 先起、ambient 后开的捕获 bug）；~200ms jitter cushion 仅 ambient 时启用→bare-TTS 首音零增延（R1）。旋钮 `campaign.barge_in_fadeout_ms`（Integer NOT NULL server_default 100；100≈真人收尾、15-30ms 只去爆音、0=旧硬切），plumb 经 `RuntimeConfig.barge_in_fadeout_ms`→`TelephonyClient.set_barge_in_fadeout`（基类 no-op + RtcTelephonyClient override，mirror configure_ambient）→ run_loop 在 configure_ambient 后调用。**部署**：①common scp 3 文件+迁移 → `alembic upgrade head`（`c4e6a8b0d2f3 → f0a1b2c3d4e5`，2 campaign 回填 100）；②engine scp `realtime/{ambient,rtc_telephony,telephony_client}.py`+`run_loop.py`+`runtime_config.py`（run_loop 部署前 md5 实证 box `b43a53e0`==git `c5a07df` 无漂移，部署后 grep prewarm 15 hits 仍在=未 clobber 并行 work）；③api scp `schemas.py`（`CampaignNestedUpdate` hand-kept extra=forbid mirror 补 `barge_in_fadeout_ms`，否则 PATCH 422——同上次 hotfix 教训，部署后 openapi 探针实证字段在）；④web rebuild（`CampaignDetail-86mBisvx.js`）+ nginx reload。探针：engine `isales_engine_started`+`credentials_loaded count=6` 无 traceback、`DEFAULT_BARGE_IN_FADEOUT_MS=100`/`set_barge_in_fadeout`/`RuntimeConfig.barge_in_fadeout_ms` 全在；api openapi 含字段。commit common `e1d8bf9`(0.8.23) / engine `59b2fae` / api `7bc2ec4` / web `ab2ae59` / meta `1c26398`+本条。本机测试 common 192✓ / engine 484✓(1 pre-existing gating fail) / web 90✓+typecheck / api campaign 21✓(2 pre-existing redis-steal)。**真机验收 deferred（§4：真机 mic 听硬切 vs 100ms 淡出 + AEC 回声）→ change 仍 active**。

---

**[历史]** 2026-06-17 16:53 CST — **engine-wrap-up-bypass-referee (Slice 2) deployed（common 0.8.21→0.8.22 + alembic `c4e6a8b0d2f3` + engine 4 文件 + api `schemas.py` 镜像 + web 重构建；engine/api/scheduler/worker 全重启）**。收尾期 opt-in 旁路裁判：客户在告别后开口但无实质新问题（`无问题`）→ 复用 `tool:hangup → END` 提前主动挂（reason=`wrap_up_referee_hangup`、复用 `HangupCause.REFEREE_HANGUP`、free-form reason 零 schema 漂移）；非门控（收尾回复照常播、played 后才消费裁判）、fail-open=不挂、barge-in 取消。开关 `campaign.wrap_up_referee_enabled`（Bool NOT NULL server_default false，**默认关、对存量零影响**）。裁判 prompt/类别为引擎内建常量（`WRAP_UP_HANGUP_CATEGORIES={"无问题"}`，不进 routing_rules）。**部署**：①common scp `models/campaign.py`+`schemas/campaign.py`+`pyproject.toml`+迁移 → `ISALES_DATABASE_URL=... alembic upgrade head`（`b3d5f7a9c1e2 → c4e6a8b0d2f3`，2 campaign 回填 false）；②engine scp `providers/llm_mock.py`+`runtime_config.py`+`wrapup/manager.py`+`run_loop.py`；③api scp `schemas.py`（`CampaignNestedUpdate` 镜像补字段，否则 PATCH 422——Slice 1 hotfix 教训，本次部署后 openapi 探针实证字段在）；④web rebuild（`CampaignDetail-DWppOuzW.js`）+ nginx reload（http 200）；engine/api/scheduler/worker 全重启（active、`isales_engine_started`+`credentials_loaded count=6` 无 traceback）。部署后探针：engine `WRAP_UP_HANGUP_CATEGORIES`/`wrap_up_referee_hangup`/`WrapUpConfig.referee_enabled` 全在、common+api schema 字段全在。**⚠️⚠️ 跨 session 部署撞车（已和解）**：与并行 greeting-prewarm 部署**同时改 `run_loop.py`**（两改动不重叠：prewarm 改开场白/listen-pump，wrap-up 改收尾轮）；我 preflight 时盒子是 Slice1（`89cb2a21`、prewarm 未上），故先 scp 了「剥离 prewarm 的 wrap-up-only」版（`c6d442da`）→ **覆盖了并行刚部署的 prewarm**。发现后经用户授权 scp **main HEAD 完整 `run_loop.py`**（`c5a07df` = `b43a53e0` = prewarm + wrap-up **都有**）→ 重启 engine，**盒子 run_loop 现 == git main HEAD**（实证 prewarm=True + referee=True）。**教训：单文件 scp + 多 session 并行改同文件 = 互相覆盖；改前必 md5 比对、部署后必 grep 验己方+他方改动都在；run_loop.py 是高频并行热点文件**。commit common `dddf205`(0.8.22) / api `edcfa4a` / engine `c5a07df` / web `0329dc7` / meta `1326fb3`+本条。测试本机 common 192✓ / engine 476✓(2x，仅 pre-existing gating flake) / web typecheck+vitest 90✓ / api campaign 21✓(2 pre-existing redis-steal)。**⚠️ §7.2 真机验收 pending**（开 `wrap_up_referee_enabled` 的 campaign 拨测：收尾客户应一句无实质内容→提前挂 reason=`wrap_up_referee_hangup`、`/calls` 不 500）→ 过了才 archive；openspec change 仍 active。**回滚**：关 `wrap_up_referee_enabled`（默认即关，零触发）；代码回滚 scp 旧 4 engine + 旧 common/api + 重启；DB `alembic downgrade b3d5f7a9c1e2`；web 回 `index.html.bak-20260617b`。

Prior: 2026-06-17 16:46 CST — **engine-greeting-window-prewarm deployed（engine 单文件 `run_loop.py` scp + 重启 engine；纯 engine，无 common/alembic/api/web）**。修「开场白与第一句 AI 回复间隔长、之后正常」。根因（多智能体诊断 wsf0qkv7e + 对抗验证）=开场白播放那几秒空闲什么下游连接都没预热，turn-1 独扛一次性建连开销：固定开场白不调 LLM → turn-1 `chat_stream` 是 main 槽首次发请求付 TCP+TLS 握手 ~100-200ms；ASR WS 懒连且 `_start_listen_pumps` 在 `_do_greeting` 之后 → 首句等 WS 现连+配置帧（落在 `processing_start` 前、pipeline_trace 测不到）。冷的是网络连接非对象（provider call 开始即 eager 构造）。**改动**：`_start_listen_pumps(start_monitors=False)` 把 ASR+ev pump 前移到开场白**前**启动（生成器推进触发 WS connect 与开场白并行、真音频保活），barge-in 两 monitor 拆出 `_start_monitors()` 开场白**后**启（保开场白不可打断）；`_drain_greeting_era_asr()` 开场白后丢弃 greeting-era ASR 结果（开场白期语音不被当首句）；`_kick_main_llm_prewarm()` 仅**固定模板**开场白 fire `chat([user:hi],max_tokens=1)` 预热 main 槽连接池（LLM 开场白自带 warm 不发冗余 probe），fail-soft 单层（吞错回落懒连）。**部署**：**部署前 md5 实证盒子 `run_loop.py` `89cb2a21...` == 我的 base `d3c6c9e`（纯增量、无 wrap-up-bypass 孤儿改动混入）**；scp 的是 **committed `c87e98c` 版**（`git show c87e98c:...`，非工作区——工作区含上个 session 遗留未提交的 `engine-wrap-up-bypass-referee` 孤儿改动）→ chown isales:isales（部署后 md5 `430dd5c7...`）→ 重启 engine（active、`cloud_edge_grpc_server_started`+`credentials_loaded count=6`+`isales_engine_started` 无 traceback）。备份 `/opt/isales/backups/run_loop.py.bak-20260617-prewarm`。commit engine `c87e98c` / meta `ee1adce`（propose+apply）+ 本条 STATE + archive。测试本机 engine **476 passed / 1 pre-existing**（`test_gate_fails_open_to_main_on_referee_timeout`，baseline stash 验 3/3 红无关）；修了 1 个被本 change 行为改变影响的 wall-clock 测试。**⚠️ 真机验收 deferred → `pipeline-stream-realmachine-acceptance`**：拨测对比同一通 turn-1 vs turn-2 的 `main_first_token_ms`/`first_audio_ms` 应显著收窄、ASR 首 finalize 不含 WS 握手（这俩字段 api `PipelineTraceRead` 未暴露，**必须直连 PG 读 `pipeline_trace`**）。Open Question：开场白时长是否短于 ASR vendor idle 断连阈值（若证伪按 design D2 加保活静音帧）。**⚠️ engine 工作区仍有遗留未提交孤儿 `engine-wrap-up-bypass-referee` 改动（openspec 无此 active），本次未动未部署。** **回滚**：scp `/opt/isales/backups/run_loop.py.bak-20260617-prewarm` 回 → 重启 engine（纯单文件、无 DB/common）。

Prior: 2026-06-17 15:27 CST — **hotfix: campaign「场景」保存 422 `Extra inputs are not permitted`（api `CampaignNestedUpdate` 漏镜像 `wrap_up_silence_hangup_ms`）**。根因=14:45 那次 engine-wrap-up-silence-hangup 部署的**残留缺口**：该 entry 记「api 无代码改动，仅 venv 刷 common」是**错判**——`PATCH /campaigns/{id}` 端点绑的是 **api 本地手维护镜像 `isales_api/schemas.py:CampaignNestedUpdate`**（每个字段重列成 optional、**不继承 `CampaignBase`**），不是 common 的 `CampaignUpdate`；重启 api 拾到的是 common 新 `CampaignBase`，但端点根本不用它 → web 保存带新字段 `wrap_up_silence_hangup_ms` 被 `extra="forbid"` 拒成 422（POST/create 不受影响，`CampaignNestedCreate` 继承 `CampaignBase` 自动带字段）。**修**：`CampaignNestedUpdate` 补 `wrap_up_silence_hangup_ms: int | None = Field(default=None, ge=0)`（bounds mirror CampaignBase）。**无迁移**（列早随 alembic `b3d5f7a9c1e2`/0.8.21 建好）、**无 openspec change**（字段已在 spec/common，纯补 api 镜像 bug）。**部署**：scp `isales_api/schemas.py`（部署前 md5 实证盒子文件 `176409e2...` == 我 main 改动前基线，纯增量）→ 重启 api（active）→ **运行态探针**：盒子 venv `CampaignNestedUpdate.model_validate({wrap_up_silence_hangup_ms:6000})` OK + 全 48 web payload 字段无 missing；**运行中 server** `curl /openapi.json`(200) 的 `CampaignNestedUpdate.properties` 含该字段=True。commit api `673697a`（已 push origin/main）；无 common/engine/web/alembic 改动。**教训**：「加 CampaignBase 字段 → 重启 api 即可」是**假信念**，必须同步手改 `CampaignNestedUpdate`（该文件 104-130 行注释早警告此漂移类，ambient/auto_restructure/tools 都踩过）。根治选项=让 `CampaignNestedUpdate` 从 `CampaignBase` 自动派生全 optional（未做，另起 change）。

Prior: 2026-06-17 14:45 CST — **engine-wrap-up-silence-hangup (Slice 1) deployed（common 0.8.20→0.8.21 + alembic `b3d5f7a9c1e2` + engine 3 文件 + web 重构建；engine/api/scheduler/worker 全重启）**。修 call-215「AI 说完再见后客户沉默，引擎不挂、播『你好还在么』硬等客户挂机」。根因=收尾静音判定 phase-blind。**改动**：收尾期（`session.in_wrap_up`）客户静默达 `campaign.wrap_up_silence_hangup_ms`（新列，Int NOT NULL server_default 6000）即主动挂断、**跳过重新激活阶梯**，发 `HangupEvent(reason="wrap_up_silence", initiated_by="ai")`（复用 `HangupCause.SILENCE_MAX_REACHED`，free-form reason 不碰封闭 `wrap_up_completed.reason` Literal）。复用既有静音机制按阶段分支（不新建第二条静音线）。计数器 `wrap_up_max_rounds/seconds` 原样保留兜底。**含 D3 计时修复**：`_await_user_or_silence` 收尾期 `asyncio.wait` 超时改用 `wrap_up_hangup_ms`（否则窗口在更短的 `silence_threshold_ms` 就醒、累加不到 wrap-up 阈值 → 挂断永不触发）。全局生效、无 per-campaign 开关。**部署步骤全绿**：①common scp `models/campaign.py`+`schemas/campaign.py`+`pyproject.toml`+迁移（**部署前 md5 实证盒子 2 common + 3 engine 文件 == 我的 main 改动前基线，纯增量不覆盖 dingrtc-migration-cloud 分支**）→ `ISALES_DATABASE_URL=... alembic upgrade head`（`a2c4e6b8d0f2 → b3d5f7a9c1e2`，加 `wrap_up_silence_hangup_ms` Int NOT NULL server_default 6000，2 个存量 campaign 回填 6000 实证）；②engine scp `realtime/silence_detector.py`+`run_loop.py`+`runtime_config.py` → 重启 engine（active、`isales_engine_started`+`credentials_loaded count=6`+`cloud_edge_grpc_server_started` 无 traceback）；③**重启 api+scheduler+worker**（共享 editable common，旧进程持旧 schema 内存→web PATCH 带新字段会 422，必须重启拾新 `CampaignUpdate`；三者均 active）；④web `npm run build`（新 `CampaignDetail-CdyRg2M9.js`）→ scp `dist/assets/*`+`index.html` → `/var/www/isales-web/` → `nginx -t`+`-s reload`（http 200、新 chunk 200、index 引用 `index-D2oaRIjJ.js`）。**部署后探针**：盒子 venv 实证 `wrap_up_silence_hangup_ms` 在 CampaignBase/Update/Read+model column 全 True；engine `evaluate_silence` 有 `in_wrap_up`、`SilenceConfig` 有 `wrap_up_hangup_ms`，wrap-up @4s→wait（不重新激活）/@6s→hangup phrase=None 实证正确。commit common `0afdffa`(0.8.21) / engine `d3c6c9e` / web `5282204` / meta `0897fe8`+`2baafcb`（api 无代码改动，仅 venv 刷 common 到 0.8.21）。测试本机：common 192✓ / engine 465✓（+5 新测）/ web vitest 90✓+typecheck✓；唯二 pre-existing fail（engine gate timeout + api redis-steal）stash+ps 甄别无关。**⚠️ §6.2 真机/真通话验收 pending**（需真拨测验收尾静默→挂断 reason=`wrap_up_silence`、无「你好还在么」、`/calls` 不 500）→ 过了才 archive；openspec change `engine-wrap-up-silence-hangup` 仍 active。**Slice 2（旁路收尾裁判判无心问题/同意挂断）未起**。**回滚**：scp 旧 3 engine 文件（`git show d3c6c9e~1:isales_engine/...`）+ 旧 2 common 文件（`git show 0afdffa~1:...`）+ 重启 engine/api/scheduler/worker；DB `alembic downgrade a2c4e6b8d0f2`（删列）；web scp 回 `index.html.bak-20260617`。

Prior: 2026-06-16 16:37 CST — **engine-ambient-background-mix deployed（common 0.8.17→0.8.18 + alembic `a2c4e6b8d0f2` + engine 5 文件 + api 1 文件 + web 重构建 + 示例背景音素材）**。出向通话叠持续背景环境音（让客户侧有真人坐席环境感），实现=出向常驻混音泵：`realtime/ambient.py`（素材 16k mono int16 预载内存 + 无缝 loop + pure-array int16 混音不依赖 audioop）+ `_CallState` 泵（10ms 墙钟绝对调度 + playout_q + barge-in 清队列泵续推背景）。**断句隔离硬不变量**：背景音 outbound-only，入向 ASR/断句 代码零改动。**双模式由 config 选非 fallback，`campaign.ambient_audio` 空时走原直推路径字节级不变**（默认关闭、对存量 campaign 零影响）。**部署步骤全绿**：①common scp `models/campaign.py`+`schemas/campaign.py`+`pyproject.toml`+迁移 → `alembic upgrade head`（`d1e2f3a4b5c6 → a2c4e6b8d0f2`，加 `ambient_audio` varchar NULL + `ambient_gain` float NOT NULL server_default 0.1，asyncpg 实证两列已建）；②engine scp `realtime/ambient.py`(new)+`realtime/rtc_telephony.py`+`realtime/telephony_client.py`+`run_loop.py`+`runtime_config.py`（**部署前 md5 实证盒子 4 文件 == 我的 main base `00b4c48`，纯增量不覆盖 dingrtc-migration-cloud 分支改动**）→ 重启 engine（active、`isales_engine_started`+`credentials_loaded count=6` 无 traceback）；③api scp `schemas.py` → 重启 api（active，schema 探针 ambient 字段在 update+base）；④web `npm run build` → 清旧 assets + scp `dist/{index.html,assets}` → `/var/www/isales-web/` → `nginx -t`+`-s reload`（新 `CampaignDetail-CzZ-TgnN.js` 就位、http 200）；⑤示例素材 `/opt/isales/ambient/office.wav`（12s 16k mono 室内底噪 RMS≈5000，gain 0.1 后≈500 RMS 轻柔背景）+ 引擎内 `get_ambient_loop` 实证可加载/拒路径穿越。**素材目录默认 `/opt/isales/ambient`（engine `ambient._asset_dir()` 默认值，无需设 `ISALES_AMBIENT_DIR`）**。commit common `dd2c359`(0.8.18) / engine `ecf9e34` / api `160f6fa` / web `d0cf6c7` / meta `aaae8e6`。**后续(2026-06-16 16:45)**：web 把素材「填文件名」改成**启用开关 + 场景下拉**（`AMBIENT_PRESETS`：`quiet_room.wav` 安静室内 / `office.wav` 办公室 / `callcenter.wav` 呼叫中心），web `28c7681` rebuild+部署（`CampaignDetail-Caobvw2n.js`）；`/opt/isales/ambient/` 现有这 **3 个合成占位素材**（12s 16k mono，tanh 软限幅无硬削，RMS≈2377/4479/5531，gain 0.1 后≈237/447/553），引擎 `get_ambient_loop` 三个均实证可加载。**再后续(2026-06-16 17:15)**：用户提供真实办公室录音 `~/Downloads/preview.mp3`（922s）→ 本地 `miniaudio` 解码（盒子/本地均无 ffmpeg）→ 16k mono int16、裁 60s、归一 RMS≈4619（gain 0.1≈461）、tanh 软限幅 → 覆盖 `/opt/isales/ambient/office.wav`（1.92MB），删掉合成的 `quiet_room.wav`/`callcenter.wav`；**重启 engine 清 `_loop_cache`**（按文件名缓存，不重启用旧素材）。web `AMBIENT_PRESETS` 精简成单选「办公室」（web `919e31b`）。素材目录现仅 `office.wav`。测试本机：engine 455 passed(+19 test_ambient) / api 21 campaign / web 90 / common 校验通过；2 处 pre-existing fail（engine gate timeout + api redis-steal）stash 甄别无关。**⚠️ 未在任何 campaign 启用**（opt-in，§7.3 真机回声实测前不该在真实外呼 campaign 上开）。**剩**：真人拨测验回声（开背景音后入向 ASR 无误 finalize/误 barge-in + 客户侧持续听到背景 + TTS 无变调）→ 过了才 archive。**回滚**：scp 旧 5 engine 文件（`git show 00b4c48:isales_engine/...`）+ 旧 api `schemas.py`+旧 common 2 文件（`git show 89bbbee:...`）+ 重启 engine/api；DB `alembic downgrade d1e2f3a4b5c6`（删两列）；web scp 回 `index.html.bak-20260616`。openspec change `engine-ambient-background-mix` 仍 active。

Prior: 2026-06-15 17:08 CST — **TTS emotion_scale=2 deployed（common 0.8.16→0.8.17 单文件 scp + 重启 engine；无 alembic）+ 双向流式 TTS 可用性已实测探明**。**改动**：`tts_volcengine.py` 给 V3 单向 SSE 请求的 `req_params.audio_params` 加 `emotion_scale`（构造参数默认 `DEFAULT_EMOTION_SCALE=2`，vendor 默认 ~4 偏高）—— 压低逐句独立冷合成下的语调/情绪摆幅，缓解跨句「音色起伏」（根因=每句一个独立无上下文 SSE 请求，韵律每句重置；见 memory `project_tts_timbre_and_protocol_history`）。**部署**：scp `isales_common/providers/tts_volcengine.py` → `/opt/isales/current/isales-common/...`（editable，共享 venv）→ **上线前实证**用部署后代码 + CredentialStore 解密凭据直接合成 `xiaohe + emotion_scale=2` 单向 = **88904 字节 ≈ 2.78s 出音正常（无回归）** → 重启 engine（active、`isales_engine_started`+`credentials_loaded count=6` 无 traceback）。commit common `89bbbee`(0.8.17)。测试 common `test_tts_volcengine.py` 13 passed（+1 emotion_scale 断言）。**🔑双向流式探针结论（`isales-engine/scripts/tts_bidi_probe.py`，ECS 实测）**：双向端点 `wss://openspeech.bytedance.com/api/v3/tts/bidirection` 在本账号**可用、能出音**，但 **resource_id ↔ speaker 严格绑定**：`seed-tts-2.0` ↔ `*_uranus_bigtts`（xiaohe/vv，**现生产音色，只能单向 SSE**，配双向端点握手通过但声学层不跑=零音频只回 SentenceStart/End 元数据）；`volc.service_type.10029` ↔ `*_moon_bigtts`/`*_wvae_bigtts`（shuangkuaisisi/M392，**能双向**，4 句单 session 连续合成 410KB/≈12.7s 实测成功，audio=`AudioOnlyServer 0b1011` raw PCM）。交叉组合一律 `55000000 resource ID is mismatched with speaker`。**含义：迁双向治本（整轮一 session 共享韵律）不需新 grant，但必须换音色到 moon/wvae 家族（xiaohe 不能双向）**——属 openspec change 范畴（engine 逐句→会话 + common 新 WS 路径 + voice 配置 + barge-in 中途关 session），尚未起。**回滚 emotion_scale**：scp 旧 `tts_volcengine.py`（`git show 675a192:...`）+ 重启 engine（纯单文件、无 DB/alembic）。

Prior: 2026-06-15 15:07 CST — **engine-filler-gated-restructure deployed（engine 4 单文件源码 + 重启 + camp1 门控 prompt DB 更新；无 alembic / 无 common 版本变更）**。让「被打断后自动重组」只在**门控判 `FILLER`（无营养垫词打断）**时触发，实质提问/反对打断改为正常作答。**根因**：`engine-auto-restructure-on-interrupt`（archived 2026-06-15）的 override 把**所有**未命中显式 routing rule、落到 `decide()` 缺省 continue 的打断都改判 restructure —— 但它 proposal 原意是「只有没营养的插话才自动续说」，从没真判「有没有营养」；客户用实质问题/反对打断、又恰好没有对应显式规则命中时被错误重组（AI 顺着说完不回答）。**修法**：①门控（主裁判）prompt 新增保留类别 `FILLER`（仅 `{{was_interrupted}}=是` 时成立）+ 引擎注入 `{{was_interrupted}}`/`{{interrupted_reply}}` 占位符；②`PipelineConfig.restructure_gate_category="FILLER"`；③`run_loop.py` override 收紧成 `gate_filler(任一 referee category==FILLER) AND continue AND 开关 AND slot AND 残句` → restructure，否则 `elif 开关 AND 残句` → 清空 `interrupt_remaining_text`（D6 跨轮残句卫生：实质打断答完即弃残句，防泄漏到后续轮误重组）；`decide()` 仍纯函数。**关键实现**：override 用 `any(category==FILLER)` 而非 by-label —— camp1 referee label 是中文「主门控」非 `main_judge`，by-label 会取不到使 camp1 永不重组；FILLER 是保留类别仅主门控定义、any 安全。**部署**：scp engine `prompt_builder.py`/`run_loop.py`/`referee.py`/`providers/llm_mock.py` → `/opt/isales/current/isales-engine/isales_engine/`（共享 venv）→ 重启 engine（active、`isales_engine_started`+`credentials_loaded count=6` 无 traceback）；camp1（id=1，已开 `auto_restructure_on_interrupt=t` + 有 restructure slot）门控 prompt（`prompt_version` id=9）经 dollar-quoted SQL 原地 UPDATE 为 HANGUP/SUCCESS/CONTINUE/FILLER 版（1601 字符、含 FILLER+两占位符），旧内容备份 ECS `/tmp/camp1_referee_id9_backup.txt`。**smoke**：deployed `restructure_gate_category` default=FILLER / `gate_filler` in run_loop / imports OK；engine active；camp1 `auto_restructure=t` + restr_slot=1；routing_rules `SUCCESS→closing` / `HANGUP→hangup`（无 transfer，FILLER 不入 routing 走 override）。commit engine `9484274` / api `f50f5de`(seed `REFEREE_PROMPT` 加 FILLER) / meta `586758d`(propose+apply)+本条 STATE。测试：engine **436 passed**（+新增 fires-when-FILLER / skipped-when-substantive+清残句 / placeholder-inject；1 pre-existing gate `test_gate_fails_open_to_main_on_referee_timeout` 失败无关 —— stash 验同红）。**openspec change `engine-filler-gated-restructure`**（MODIFY ai-pipeline「被打断自动重组开关」requirement：收紧为门控判 FILLER + 占位符注入 + 清残句不变量）。**真机验收待做**（①垫词打断「嗯嗯你说」→ AI 顺着说完 ②实质打断「那多少钱」→ AI 正面作答不重组 ③非打断不受影响，主观需用户拨测）→ archive 待真机。**回滚**：scp 旧 4 文件（`git show 87c8792:isales_engine/...`）+ 重启 engine + camp1 prompt 用 ECS `/tmp/camp1_referee_id9_backup.txt` dollar-quoted UPDATE 回 id=9；纯单文件 + 1 行 DB，无 alembic / 无 common。

Prior: 2026-06-12 17:10 CST — **engine-main-native-multiturn deployed（engine 单文件源码 + 重启；无 alembic / 无 common 版本变更）**。修「主对话衔接不自然」。**根因**：main 流式 LLM 的消息组装把整通对话拍平成单条 user message（`prompt_builder.py:_render_dialog` 用 `用户:`/`AI:` 文本前缀 + 末尾 `AI:` 续写钩子），这是从 referee 旁路分类器格式误移植到主线 —— 范本 voxen 恰相反：主对话线 `core/llm/chat_history.go:GetHistory` 用原生多轮，仅 referee `ChatHistoryToString` 拍平。chat 模型对话连贯性在原生多轮上训练，拍平 = 退化成文本补全。**修法**：`build_main_messages` 改为返回 `[system] + dialog_history 1:1 映射的多轮 user/assistant message`（`DialogTurn.role` 已是 user/assistant 直接映射）；会话级静态上下文（上次纪要 + 线索信息）移入 system message（context 而非对话轮），放主体后/指令前以保 `WRAP_UP_APPEND` 最末；FOLLOW_UP 措辞「下面的」→「上述」（纪要现位于其上）；删 `_render_dialog`/`_build_user_message`（无外部调用方，referee 有独立 `_render_dialog_history_for_referee` 不受影响）。**部署**：scp `prompt_builder.py` → `releases/20260517-222944/isales-engine/isales_engine/pipeline/`（editable，共享 `/opt/isales/current/venv`）→ 重启 engine（**仅 engine**：纯 prompt 组装，api/scheduler/worker 无关）。**smoke**：deployed module `build_main_messages` present / `_render_dialog` removed / `_render_context` present；engine `active`。commit engine `59a1a9d` / meta `6e22ba0`（propose+apply §1-3）+ 本条 STATE。测试：engine **421**（415+6 新 `test_prompt_builder.py`；1 pre-existing gate `test_gate_fails_open_to_main_on_referee_timeout` 失败无关 —— stash 后原 main 代码 5/5 同红，本机 20ms 超时过紧的时序 flake）。**走 openspec change `engine-main-native-multiturn`**（MODIFY role-prompt 三 requirement，反转「不使用→使用 multi-turn」）。**真机验收 §5 待做**（§5.1 mac smoke + §5.2 真人 mic 改前后衔接对比，主观需用户）→ archive 待 §5。**回滚**：scp 旧 `prompt_builder.py`（git show `b91502b:...`）+ 重启 engine；纯单文件、无 DB / 无 common 牵连。

Prior: 2026-06-12 17:09 CST — **engine-transcript-write-validation deployed（engine 写时校验 transcript 事件契约；纯 engine 单文件 + 重启，无 common/api/web/alembic/DB）**。根治上一/下一条(wrap_up reason)所属**系统性根因**：`CallSession.append_event` 只校验判别符(`event_type in TRANSCRIPT_EVENT_TYPES`)、从不校验**字段契约** → 引擎-schema 漂移(错 literal/越界 key/缺字段)被静默持久化进 `call_record.transcript`，只在 api 读回(`CallRecordRead` extra=forbid)炸成 `/calls` 500；同根因已 3 次生产事故(06-10 `ai_reply.interrupted`+`interruption.*`、06-12 `wrap_up_completed.reason`)。**修法**：`append_event` 组完 event dict 后、append 前用模块级 `TypeAdapter(TranscriptEvent).validate_python` 全字段校验；违例**永远** `logger.error("transcript_event_schema_violation", type, errors)`，**仅严格模式**(`ISALES_ENGINE_STRICT_TRANSCRIPT=1`)抛异常。**fail-fast-in-CI / fail-soft-in-prod**：生产(env 未设)只记日志+仍持久化(transcript 是观测数据，日志契约 bug 不得拖垮 live 通话)；引擎测试 conftest 设 strict=1 → **全部 425 测变 drift 探测器**。保留既有 unknown-type ValueError 守卫；**刻意不放宽 api 读取侧**(那是掩盖数据腐败)。永久写时不变量、非兜底(只暴露错误、无替代成功路径)。**部署**：scp engine `call_session.py`(共享 `/opt/isales/current/isales-engine`，仅此一文件入 prod；conftest+测试 repo-only)→ 重启 engine。**smoke**：部署源含 `_TRANSCRIPT_EVENT_ADAPTER`(L41)+校验块(L223)+`transcript_event_schema_violation`(L226)+strict gate(L235)；engine `active`、`cloud_edge_grpc_server_started`+`isales_engine_started` clean、无 schema_violation；**`ISALES_ENGINE_STRICT_TRANSCRIPT` 在 prod engine.env 未设(grep=0 → fail-soft，正确)**。测试：engine **425 passed**(strict 全开、+4 新测、**无任何新 strict 失败 = 全引擎测试路径无其它潜伏漂移**；1 pre-existing `test_gate_fails_open_to_main_on_referee_timeout` 无关)、ruff+mypy clean；common 189(未改)。commit engine `423b2f9` / meta：本条 STATE + openspec apply + archive。**openspec `engine-transcript-write-validation` 本 session apply+archive**(纯 robustness、无真机依赖，acceptance=测试网绿+部署健康)。**回滚**：scp 旧 `call_session.py`(恢复仅判别符校验)+ 重启 engine（无 DB/无 alembic/无 common）。

Prior: 2026-06-12 16:00 CST — **fix transcript wrap_up_completed.reason schema drift deployed（common 数据迁移 + engine 源码修复；生产 1 行迁移 + engine 重启）**。修「`http://121.89.85.150/calls` 外呼记录页报错」= `GET /calls` 500。**根因**：engine 收尾评估器 `wrapup/manager.py:evaluate_wrap_up` 把它的**内部控制词** `rounds_exhausted`/`seconds_exhausted` 经 `run_loop.py:1574 append_event("wrap_up_completed", reason=…)` **原样持久化**进 transcript 事件，但 transcript spec § 事件表 + `WrapUpCompletedEvent.reason` 约束为 `Literal["max_rounds","max_seconds"]`；`CallRecordRead` 用 `extra="forbid"` 校验，**库里只要 1 条这样的行整张 `/calls` 列表就 500**（同 06-10 transcript schema drift 同类、新实例）。**全量审计先行（吸取 06-10「先审计 ALL 越界再动手」教训）**：①生产库全量 drift 审计（186 通 / 1248 事件，用部署的 `TranscriptEvent` union 逐事件校验）→ **仅 1 条 drift**：call **173** `wrap_up_completed.reason='rounds_exhausted'`，无其它隐藏越界 key/literal/type；②engine 静态审计（workflow：23 个 transcript 写入点全枚举 + 对抗 verify）→ 除此 `reason` 外**无任何其它写入点会产生越界值**（`hangup.initiated_by` 等 literal 字段全合规）。**修法（遵循「避免多层兜底」）**：在源头改 `manager.py` 两个 `WrapUpDecision.reason` 直接产出 spec 词汇 `max_rounds`/`max_seconds`（**不在 run_loop 写入边界加映射层** —— reason 唯一生产消费者就是那行 append_event，加映射 = 永久双词汇债），同步改 2 个单测 + golden fixture（`goal_achieved_wrapup.json:108` 原 pin 旧坏值）；common 加**数据迁移 `c0d1e2f3a4b5`** 重写存量（`rounds_exhausted→max_rounds`、`seconds_exhausted→max_seconds`，幂等、保序 WITH ORDINALITY、仅 touch 违规行；**downgrade 是 no-op** —— max_rounds/max_seconds 是 canonical，回退会重新弄坏并污染修复后引擎写的正确行）。common 0.8.14→**0.8.15**（纯加迁移、无 model 变更，consumer pin `<0.9` 已涵盖无需改 pin）。**alembic `b9c0d1e2f3a4 → c0d1e2f3a4b5`**。**部署**：scratch DB 跑全链路 upgrade + 数据变换 + 幂等性先验全绿 → scp common(migration + pyproject)+engine(`wrapup/manager.py`) editable（`current`=releases/20260517-222944 共享 venv，api/engine/scheduler/worker 同 `/opt/isales/current/venv`）→ `pg_dump -t call_record`→`/opt/isales/backups/wrapup-reason-call_record-20260612-160044.sql`（329 行）→ `alembic upgrade head`（current=c0d1e2f3a4b5）→ 重启 engine（**api 无需重启**：纯数据修，api 每请求重查 DB，迁移后即合规）。**smoke（决定性实证）**：call 173 reason `rounds_exhausted`→**`max_rounds`**；部署的 `manager.py` 含 spec 词汇；**全量 drift 审计 `rows that FAIL validation: 0` / `NO DRIFT`**；**`CallRecordRead.model_validate`（端点同款全字段校验）`186/186 OK → GET /calls will not 500`**；api journal 无新增 /calls 500；engine `active`。commit common `ee65276`(0.8.15) / engine `b91502b` / meta `<pending>`(STATE)。测试：common **189** / engine **415**（1 pre-existing gate `test_gate_fails_open_to_main_on_referee_timeout` 失败无关，diff 仅触 wrapup，确定性同红非 flake）。**纯 conformance bugfix，无 spec delta（transcript spec 早已写 max_rounds/max_seconds），未走 openspec change**。**回滚**：`alembic downgrade c0d1e2f3a4b5 → b9c0d1e2f3a4` **不恢复数据（downgrade no-op）** → 要真回滚数据须 restore `wrapup-reason-call_record-20260612-160044.sql`；engine 回滚 = scp 旧 `manager.py`（恢复 rounds_exhausted/seconds_exhausted）+ 重启 engine。

Prior: 2026-06-12 15:33 CST — **engine-filler-delay-client-anchor deployed（engine 单文件源码 + 重启；无 alembic / 无 common 版本变更）**。修垫词触发时机不贴客户侧。**根因 = spec-code 漂移**：`filler/spec.md` + `data-model/spec.md` 早写「**进入 PROCESSING 后** `filler_delay_ms` 首音频未出就播垫词」，但 `run_loop.py:_play_streaming._maybe_filler` 把计时锚在「`_play_streaming` 进入」——gate-first 重构（多裁判门控）把 referee 投票插到 `_play_streaming` **之前**后，门控/ASR-finalize 那段静默被排除在 `filler_delay_ms` 预算外（旧实现拿「任务创建时刻」当「PROCESSING 进入」的代理，门控前移即失效），运营体感「客户没等满设定延迟垫词就播」。**修法**：`_maybe_filler` 改 `remaining = filler_delay_s - (time.monotonic() - processing_start)`（`processing_start` 已是 `_play_streaming` 入参 = PROCESSING 进入 ≈ 客户说完话），`remaining>0` 才 sleep，`remaining<=0`（门控已吃满预算）即刻播；门控等待从此计入 `filler_delay_ms`。仅 main 回合受影响（wrap-up/restructure 均 `filler=None`）。**部署**：sha 验 ECS run_loop.py==local HEAD `4b6e53dc` 在同步不 clobber → 备份 `/opt/isales/.../run_loop.py.bak-20260612-153250` → scp（ECS sha→`720dd750`）→ 重启 engine（**仅 engine**：纯播音计时；`graceful_shutdown active=0` 不丢通话；新 PID 961287 `isales_engine_started`+`credentials_loaded count=6` 无 traceback）。**smoke**：deployed `_maybe_filler` 含 `remaining = filler_delay_s - ...`；**事后复验**——native-multiturn/transcript-fix 的 16:01-16:15 并行重启后 ECS run_loop.py 仍 `720dd750`（各 change 走单文件 scp 互不覆盖，run_loop.py 未被动）。commit engine `c921d7a` / meta `4823258`(propose+apply)+本条 STATE+archive。测试：engine `test_play_streaming` 11/11（+3 门控锚点用例：门控<预算等剩余 / 门控≥预算即刻 / 快轮次取消）；全 engine **415 passed**（1 pre-existing gate `test_gate_fails_open_to_main_on_referee_timeout` 无关，stash 验 clean HEAD 同红）。**真机验收 deferred**：prod 两 campaign `filler_enabled` 均 false（camp1 `filler_delay_ms=2500` 但开关 off / camp2 空），无 live 垫词路径可拨测——待某 campaign 开垫词时真机顺带验。**openspec change archived** `archive/2026-06-12-engine-filler-delay-client-anchor`（MODIFY filler「时间门控播放」requirement 钉死锚点 + 新增「门控超预算即刻播」scenario）。**回滚**：scp `run_loop.py.bak-20260612-153250` 覆盖 + 重启 engine（纯单文件，无 DB / 无 common 牵连）。

Prior: 2026-06-12 15:20 CST — **remove-do-not-call-campaign-config deployed（common + api + web 全栈 + 生产 DROP COLUMN）**。删死配置「勿打」campaign 级三列 —— `do_not_call_keywords` / `do_not_call_llm_enabled` / `do_not_call_llm_prompt_version_id`（+ FK `fk_campaign_do_not_call_llm_prompt_version_id_prompt_version`）。审计实证（5-agent map + 8 对抗 verifier 全 claim_holds=true）：三列 stored-but-unused，engine/scheduler/worker **零运行时消费者**——关键词臂无 ASR 匹配器、独立 LLM 判定臂在 `impl-engine` 即 defer 为 mock 后从未实装、prompt FK 也无法经 `PromptVersionsSnapshot`（无槽）/ scheduler `pack_prompt_versions`（只走 role_config）抵达引擎。**保留** `LeadStatus.DO_NOT_CALL` 活路径（worker `lead_state._has_do_not_call_marker` 经 routing-rule `goal_type=do_not_call` / `do_not_call_marked` 标记产出 + web「免打扰」线索状态显示 + webhook-callback `lead.status=do_not_call` 触发）——**删的是配置入口、保的是结果终态**。**alembic `a8b9c0d1e2f3 → b9c0d1e2f3a4`**（DROP FK + 3 列；downgrade re-add）。common 0.8.13→**0.8.14**。**部署**：scratch DB up→down→up round-trip 先验 → scp common(campaign model / prompt.py 注释 / schema / pyproject / migration)+api(schemas) editable（`current`=releases/20260517-222944 symlink，inode 654239 实证 scp 穿透生效、live campaign.py `do_not_call`=0、alembic heads=b9c0d1e2f3a4）+ chown → `pg_dump -t campaign`→`/opt/isales/backups/remove-do-not-call-campaign-config-20260612-151627.sql` → `alembic upgrade head`（current=b9c0d1e2f3a4、剩 0 列、FK gone）→ 重启 api+engine+scheduler+worker。web build（entry `index-vdAeO0dN.js`、`CampaignDetail-BXdOsCCf.js`）→ 备份 `/var/www/isales-web.bak-20260612-151935` → rsync `--delete` dist → nginx reload。**smoke**：scheduler 新进程 `active_campaigns_restored count=1`+`dispatch_skip_device_select campaign_id=1`（**新 model 对迁移后 schema SELECT campaign 成功、无 UndefinedColumnError ← 决定性实证**）；api 新 PID 960516 `Application startup complete` 无 do_not_call 错（旧 PID 899461 在 DROP↔重启窗口报一次 transient UndefinedColumnError，预期）；`:8000/docs`→200、`/campaigns`→401；openapi `do_not_call`=**1**（= 保留的 `LeadStatus` 枚举；campaign schemas=0）；公网 HTTP `/api/docs`→200、`/api/campaigns`→401、SPA `/`→200；served assets `DoNotCall` 组件引用=0（`do_not_call` 仅余 useStatusMeta/LeadList 的免打扰状态）；deployed common metadata=0.8.14。commit common `a3da1c8`(0.8.14) / api `721b032` / web `e6cd9e1` / meta `0672138`(propose+apply §1-4)+archive+本条 STATE。测试：common **189** + migration round-trip / api 改动的 `test_campaigns_crud` **12**（2 pre-existing redis-steal `test_campaign_start_pause` 无关）/ web vitest **90** + tsc build / telephony **345** / worker **57**（engine 1 pre-existing gate `test_gate_fails_open_to_main_on_referee_timeout` + scheduler 3 redis-steal 均环境/无关——已查实 6 个 `_isales` 守护进程 + 3 个 blpop 偷 `engine:dial`/`campaign:control`）。**回滚**：`alembic downgrade b9c0d1e2f3a4 → a8b9c0d1e2f3`（re-add 3 列 + FK，行为回到「配置存在但无运行时消费」无害态）+ scp 旧源码（common 4 文件 + api schemas）+ 重启 + web rsync 回 `/var/www/isales-web.bak-20260612-151935` + nginx reload；或 restore `remove-do-not-call-campaign-config-20260612-151627.sql`。

Prior: 2026-06-12 11:19 CST — **engine-auto-restructure-on-interrupt §7 退役 restructure 路由动作 deployed**（common + engine + api + web，**纯代码、无 alembic / 无 DB**）。承接同 change 的开关：开关一旦存在，「用 routing rule 触发 restructure」即冗余（多路径兜底）→ 删 legacy `type=restructure` action（common `RestructureAction`/`RestructureSource` + union/export）+ `route to=restructure` 目标（api `_BUILTIN_ROUTES`→422、web 下拉/source 选择器/fail-open）+ engine decider `atype=="restructure"` 分支与 `restructure_enabled` 参数（删后 `decide()` 永不返回 kind=restructure，只有 call-site 开关 override 会）+ `_BUILTIN_THEN` 的 restructure。**restructure 从此纯开关驱动**；**保留** restructure 角色 + run_loop `kind=="restructure"` 执行分支（开关产 kind=restructure 走它）+ `max_continuous_restructure`（现基本 vestigial，标后续清理）。**部署前 diff 验 5 文件均与删除前 base 逐字节相同**（安全 overlay）→ scp common(routing_rule+__init__+pyproject 0.8.13)+engine(decider+run_loop)+api(routing_validation) → chown → import sanity（部署源 `RestructureAction` gone / `type=restructure` 被 ValidationError 拒 / `decide()` 参数仅 `[referee_results, routing_rules]`）→ 重启 engine+api+scheduler+worker。web build（entry `index-Bvjb8_tv.js`）→ 备份 `/var/www/isales-web.bak-20260612-111900` → rsync `--delete` → nginx reload。**smoke**：engine `cloud_edge_grpc_server_started` clean、api `Application startup complete`、**run_loop `kind=="restructure"` 分支仍在（开关执行路径未误伤）**、`_BUILTIN_THEN={closing,recovery}`、openapi `RestructureAction`=0、`/api/docs`→200、`/api/campaigns`→401、SPA 200。commit common `8a636f5`(0.8.13) / engine `5803f5f` / api `03f25e4` / web `9364dca` / meta `2f790d7`(retire)+本条 STATE。测试：common 189 / engine 412（+1 pre-existing gate 无关）/ api 113（+2 pre-existing redis-steal 无关）/ web vitest 90 + build。**⚠️ 仍待真机演练（task 5.6）+ archive**（验开关 restructure 真触发 + veto）。**openspec `engine-auto-restructure-on-interrupt` 未 archive**。**回滚**：scp 删除前源码（恢复 RestructureAction + decider 分支 + `_BUILTIN_THEN`/`_BUILTIN_ROUTES` restructure + web）+ 重启 + web rsync 回 `.bak-20260612-111900`（无 alembic 无 DB 回滚）。

Prior: 2026-06-12 10:30 CST — **split-model-and-speech-provider-config deployed（common + engine + api + web 全栈 + 生产凭据迁移；camp1 main 模型待用户开通豆包 + 真机验收待做）**（**无 alembic / 无表结构变更**，纯 `provider_credential` 行 provider_id 改写 + 应用层枚举）。火山(字节)两条独立产品线两套独立密钥被硬塞进同一个 `volcengine` provider_id → 主 LLM 走 volcengine 每轮 401「format incorrect」只播兜底词（per-role-llm-config 让 ext_params.provider 生效后第一次真走 volcengine 才暴露）。**根因 3 层**：①engine `factory.build_llm("volcengine")` 把语音的 `app_token` 当 ark LLM key 读 → 必然 401；②web `ModelProviderConfig.vue` 把 LLM key 与语音 token 混进同一组 `app_key`/`app_token` 字段、无独立 ark key 字段；③DB `volcengine` 一个 provider_id 混放 `api_key`/`app_key`/`app_token`/`tts_resource_id` 语义重叠。**拆法**：保留 **`volcengine`** = 火山方舟 Ark LLM（持 ark `api_key` + `endpoint` + `default_model`），新增 **`volcengine_speech`** = 豆包语音 ASR/TTS（持 `app_key`+`app_token`+`tts_resource_id`+新版 `api_key` X-Api-Key）。engine `build_llm("volcengine")` 改读 `api_key`（**修 401 bug**）；`build_asr`/`build_tts` + `KNOWN_ASR/TTS_PROVIDERS` 改 `volcengine_speech`；common `build_volcengine_tts`（engine build_tts + api 试听共用）改读 `volcengine_speech`；新增 `isales-cred-migrate split-speech` 子命令（dry-run 默认 / --apply / --rollback，幂等 + UNIQUE 冲突 SKIP 守护，provider_id 改写 cipher 不动）；api `ALLOWED_PROVIDER_IDS += volcengine_speech` + `ALLOWED_FIELD_NAMES += tts/asr_resource_id`；web 拆「模型配置」(LLM) + 「语音服务配置」(speech) 两块 field-driven 卡 + 新 `speechProviders.ts` SSOT。common 0.8.11→**0.8.12**（纯枚举/约定，consumer pin `<0.9` 已涵盖，无需改 pin）。**部署顺序（D5 原子化）**：`pg_dump provider_credential`→`/opt/isales/backups/provider_credential-split-20260612-101555.sql` → scp common(cli/cred_migrate.py + providers/tts_volcengine.py) editable → **dry-run 核对**（4 行：app_key/app_token/api_key/tts_resource_id）→ `split-speech --apply`（4 行 volcengine→volcengine_speech）→ `import-env --apply` 写 ark key `82989d4d-…2c49`→`volcengine.api_key`（**先迁移后写 key**，否则 ark key 会被当语音键搬走）→ scp engine `factory.py` + api `provider_credentials.py` editable → 翻 `/etc/isales/env/engine.env` `ISALES_ENGINE_ASR/TTS_PROVIDER=volcengine`→`volcengine_speech`（备份 `engine.env-split-*.bak`）→ 重启 engine+api（worker/scheduler 不重启：不读语音凭据/不 import 改动文件）。web build（entry `index-CJ3BqlMB.js`，`ModelProviderConfig-BeJOpsrp.js`）→ 备份 `/opt/isales/backups/web-split-model-20260612-102416.tgz` → rsync `--delete` dist → `/var/www/isales-web` → nginx reload。**迁移后 provider_credential 6 行**：`dashscope.api_key` + `volcengine.api_key`(ark) + `volcengine_speech.{api_key,app_key,app_token,tts_resource_id}`。**smoke**：engine `cloud_edge_grpc_server_started`+`credentials_loaded count=6 providers=['dashscope','volcengine','volcengine_speech']`+`isales_engine_started` clean（无 NotImplementedError → build_asr/tts 从 volcengine_speech 构建成功）、`build_llm("volcengine")` 实测构建 OK（key_prefix=8298 = ark）、ark key 实测**鉴权通过不再 401**（curl ark /chat/completions 返回 model NotFound 而非 auth-failed）、api `Application startup complete`+`/health`→200、nginx 公网 `/`→200 新 bundle。commit common `d0bf1f9`(0.8.12) / engine `2457991` / api `2eac78f` / web `abae626` / meta `068e646`(apply §1-6)+本条 STATE。测试：common **189** / engine **415**（1 pre-existing gate `test_gate_fails_open_to_main_on_referee_timeout` 失败无关，stash 验证 clean HEAD 同红）/ api **112**（2 pre-existing redis-steal `test_campaign_start_pause` 无关，stash 验证 clean HEAD 同红）/ web vitest **90** + vue-tsc clean。**⚠️ 卡点 = camp1 main 模型（账号侧，非代码）**：火山方舟账号当前**无可用豆包模型** —— `GET /api/v3/models` 列出的全是 `status:Shutdown`、`doubao-seed-1-6`→`ModelNotOpen`、`doubao-pro-32k`/`-241215`→`InvalidEndpointOrModel.NotFound`。camp1 main 配 `volcengine`+`doubao-pro-32k` → 现在 404→兜底（auth 已修好，只剩模型）。**用户决定 camp1 main 自己在「模型配置」UI 处理**（开通豆包模型/建推理接入点 ep-xxx → 设 camp1 main model）。**⚠️ LLMRegistry fail-open 只在 build 错误（NotImplementedError/KeyError/ValueError）触发、不接 runtime HTTP 404** → valid-key/bad-model 的 main 会 404→兜底而非回落 dashscope（要么修模型、要么把 main 切 dashscope）。**真机验收（§7.4）deferred**：需用户开通豆包模型 + mac/QA 拨一通验 AI 真用豆包对话。**回滚**：`isales-cred-migrate split-speech --rollback`（4 行搬回 volcengine）+ 删 `volcengine.api_key`(ark) + scp 旧源码（engine factory.py / common cred_migrate.py+tts_volcengine.py / api provider_credentials.py）+ 翻 engine.env 回 volcengine + 重启 engine+api + web rsync 回 `web-split-model-20260612-102416.tgz` + nginx reload；或 restore `provider_credential-split-20260612-101555.sql`（无 alembic 无 schema 回滚）。

Prior: 2026-06-12 09:43 CST — **engine-auto-restructure-on-interrupt deployed（backend+web 全栈；真机演练待做）**（common + engine + api + web 全栈）。加 campaign 布尔开关 `auto_restructure_on_interrupt`（默认 false）：ON 时，被 barge-in 打断的那一轮若**无任何显式 routing rule 命中**（decider 缺省落 continue）、配了 restructure slot 且 `session.interrupt_remaining_text` 非空，则在 decide() **调用点**把缺省出口改判为 `restructure(source=interrupt_remaining)`（复述被切断的尾巴）而非 continue——即「被没营养的插话打断就自动续说」。**referee 仍 gate-first 在前、显式规则 first-match-wins 当 veto**（客户真异议/转人工/拒识对应规则先命中→正常接管，自动重组只接管无命中的缺省位）。**刻意不做 output-then-gate**（重组音频必须等 ASR final 否则二次抢话；referee 每轮必跑、重组搭便车 0 额外 LLM）。`decide()` 保持纯函数（override 仅在 run_loop 调用点）。自动重组仍受 `max_continuous_restructure` 封顶；开关默认 OFF 时行为逐字节不变。**alembic `f7a8b9c0d1e2 → a8b9c0d1e2f3`**（纯加性 ADD COLUMN bool NOT NULL server_default false 回填存量行；downgrade 干净 drop）。common 0.8.10→**0.8.11**。**部署**：scp common(campaign model+schema+pyproject+migration)+engine(run_loop/runtime_config/prompt_builder)+api(schemas CampaignNestedUpdate) editable 源码（**部署前 diff 验 6 文件均与 pre-change base 逐字节相同**——engine release 虽挂 dingrtc-migration-cloud 分支标签但部署文件实与 main 同步，安全 overlay；ECS alembic 实为 f7a8b9c0d1e2，STATE 旧记的 c3d4e5f6a7b8/580b817550c8 早已 stale）→ chown isales:isales → `alembic upgrade head`（f7a8b9c0d1e2→a8b9c0d1e2f3）→ 重启 engine+api+scheduler+worker 全队列。web build（entry `index-CrRHl5l3.js`）→ 备份 `/var/www/isales-web.bak-20260612-094300` → rsync `--delete` dist → nginx reload。**smoke**：`alembic current`=a8b9c0d1e2f3；列 `auto_restructure_on_interrupt boolean NOT NULL default false`；campaign 1+2 回填 `f`；openapi 含 `auto_restructure_on_interrupt`；`/api/docs`→200、`/api/campaigns`→401、公网 SPA 200；engine `cloud_edge_grpc_server_started` clean、4 服务 active。**未做 live authenticated PATCH**（写路径已由 api `test_auto_restructure_on_interrupt_create_default_and_patch` round-trip + openapi + DB 列回读覆盖）。commit common `cb108c9` / engine `d0a5f1f` / api `4a685a0` / web `1b3be9a` / meta `9552eae`(propose+apply)+本条 STATE。测试：common 183 / engine 412（+5 auto_restructure run_loop + 1 decider purity；1 pre-existing gate `test_gate_fails_open_to_main_on_referee_timeout` 失败无关，stash 验证 clean tree 同红）/ api 111（+1 round-trip；2 pre-existing redis-steal 环境失败无关，`client list` 实证 3 blpop 守护进程）/ web vitest 90 + build。**⚠️ 待真机演练（task 5.6）**：mac/QA 拨测，垫词(「嗯」「你继续」)打断→AI 自动续说被切的话 vs 真异议(「我没空」「多少钱」)打断→AI 正常回应（验 veto）；需活体 referee + 音频链路。**openspec `engine-auto-restructure-on-interrupt` 未 archive**（待真机演练或用户决定）。**回滚**：alembic downgrade `a8b9c0d1e2f3 → f7a8b9c0d1e2`（drop 列，行为回退到「仅显式规则触发 restructure」、无害）+ scp 旧源码（恢复 6 文件 pre-change 版本）+ 全队列重启 + web rsync 回 `/var/www/isales-web.bak-20260612-094300` + nginx reload。

Prior: 2026-06-11 16:12 CST — **fix-goal-achievement-pipeline deployed（backend 全链路；真机验收待做）**（common + engine + worker + web 全栈 + campaign 1 DB 配置，**无 alembic / 无 DB schema**）。修「目标达成端到端从未记录」（camp1 160 通 call_summary 全 `goal_achieved=false`、看板 `/analytics/goal-rate` 恒 0、lead 0 completed）。三层根因：①**worker（总闸）** `llm/mock.py` `_last_role_markers`/`_stitch_text` 只读已废弃 `bot_speech`，引擎写 `ai_reply`（2026-06-10 schema-drift 规范成 ai_reply 时漏改 worker 的尾巴）→ 每通无条件清 false + 摘要丢全部 AI 回复；改读 `ai_reply`（**latch**：达成后不被 WRAPPING_UP 的 `goal_achieved=false` ai_reply 覆盖）+ 独立 `goal_achieved` 事件，测试 fixture/脚本同步改 ai_reply。②**engine** `pipeline/decider.py` `route` 动作丢 `goal_type`（仅 legacy `transition` 提取）→ 对称补 `goal_type=action.get("goal_type")`。③**camp1 配置** referee(main_judge) prompt 只产 HANGUP、SUCCESS 规则用 `tool:hangup`（命中也不进 closing route）→ prompt 加 SUCCESS（正向意向任一渠道：试听/微信/回访）+ CONTINUE、规则改 `route:closing + goal_type=intent_confirmed`。**Path A（实装暴露）**：`RoutePersonaAction` 无 `goal_type` 字段 + `AppModel` `extra=forbid` → route+goal_type 会 500 `GET /campaigns`（重演 06-10 drift bug）→ common 0.8.9→**0.8.10** 给 route 加可选 `goal_type`（validator 仅 `to=closing` 合法）；web `types/campaign.ts` + `RoutingRulesTab.vue` 加 route:closing 的 goal_type 输入 + 去 `transition(旧)`/`restructure(旧)` 下拉选项（engine 留 removal-tracked shim 执行存量）。删 camp1 orphan `tools.end_call`（零代码引用）。**部署**：scp engine `decider.py` + worker `mock.py`/`base.py` + common `routing_rule.py`（current=releases/20260517-222944，common editable 一处全端生效）→ import/schema smoke（`RoutePersonaAction(to=closing,goal_type=…)` OK、非 closing 拒；worker `_last_role_markers` ai_reply→`(True,'intent_confirmed',{})`、bot_speech→`(False,None,{})`）→ **common 先于配置写入重启 api**（否则旧 schema 校验 route+goal_type → 500）+ 重启 engine+worker。web build（entry `index-B1BszVBT.js` + `CampaignDetail-CPmAwGD8.js`）→ 备份 `/opt/isales/backups/web-fix-goal-achievement-20260611-161044.tgz` → rsync `--delete` dist → `/var/www/isales-web` → nginx reload。camp1 DB 配置经 psql 事务（referee prompt + routing_rules + 删 end_call，前后对照 + COMMIT）。**smoke**：engine `cloud_edge_grpc_server_started`+`credentials_loaded count=5 providers=['dashscope','volcengine']`+`isales_engine_started` clean、worker `isales_worker_started`+`metrics_tick_done campaigns=1`、api active + `/health`→200、**deployed common 校验 camp1 routing_rules OK（不 500，SUCCESS→RoutePersonaAction to=closing goal_type=intent_confirmed）**、nginx 公网 `/`→200 新 bundle。commit common `b356fb6`(0.8.10) / engine `59654ff` / worker `cecfa16` / web `28a961f`+`7c23cc0` / meta `a6ebf1d`+`c2f9813`。测试：common **183** / worker **57** / web 29(routing/campaign)+vue-tsc clean / engine decider+gating+run_loop **34 passed**（1 pre-existing `test_gate_fails_open_to_main_on_referee_timeout` 无关，stash 验证 clean tree 同红）；make test-all 本 change 触及范围全绿，api2+scheduler3 红=redis-steal 环境 flake（两仓未触碰、git clean）。**⚠️ 待真机验收（tasks 6.2-6.5）**：mac/QA 拨一通「成功意向」电话 → 验 referee 真吐 SUCCESS → route:closing → `goal_achieved`+`goal_type` 事件 → worker `call_summary.goal_achieved=true` → lead COMPLETED → `/analytics/goal-rate` 非 0（活体 referee LLM + 音频链路只能真机验）。**openspec `fix-goal-achievement-pipeline` 未 archive**（待真机验收）。**回滚**：scp 旧源码（engine decider / worker mock+base / common routing_rule）+ 重启；camp1 DB 配置还原（routing_rules SUCCESS→`tool:hangup`、prompt 去 SUCCESS/CONTINUE、tools 加回 end_call）；web rsync 回 `web-fix-goal-achievement-20260611-161044.tgz` + nginx reload（无 alembic 无 DB schema 回滚）。 **【16:55 追加 — referee 活体验收暴露并修复 3 个 latent blocker，referee 分类已实证全绿】** 真机验收前先做 referee 活体测试（ECS 上用部署的 qwen-turbo + camp1 prompt 跑 `run_referee`），连环挖出 3 个**卡死 referee（判不出任何类→goal/HANGUP 全失效）的非本-change 问题**：(a) **referee prompt 缺占位符** — camp1 main_judge prompt（原版+本 change 初版）都没有 `{{user_last_utterance}}`/`{{recent_dialog_history}}`，run_referee 靠它们注入对话 → referee 看不到客户说啥；改为 canonical 格式（含占位符，参 `isales-api/scripts/seed_pipeline_stream_campaign.py`）。(b) **referee provider 错配** — main_judge `ext_params.provider=volcengine` 但 `model=qwen-turbo`（dashscope 模型），volcengine 只有 ASR/TTS 凭据 → **401「API key format incorrect」→ fail-open**；是 per-role-llm-config（今早部署让 ext_params.provider 生效）暴露的数据错配，改 `provider=dashscope`（对齐同用 qwen-turbo 的 extractor）。(c) **⚠️ 部署引擎 `referee.py` 滞后 main** — ECS `releases/20260517` 的 referee.py `_USER_PRIMER` 仍是旧版 `"请只输出一个词：pass 或 hold…"`（user 消息硬逼模型只吐 pass/hold，**压过 system prompt 的任何枚举 → 本引擎上所有多类别 referee 全废**）；main 早改成通用 `"请按系统提示只输出一个分类词…"`（diff 仅此一行 + 注释）→ scp main referee.py + 重启 engine（md5 `d01e48de…`）。修完 **referee 活体测试 4/4 全绿**：正向意向/加微信→SUCCESS、明确拒绝→HANGUP、正常询问→CONTINUE（provider=dashscope qwen-turbo temp=0.2）。**goal_achieved 判定+记账链路功能上端到端打通**（referee 实证 + 下游确定性代码已验）；剩**真机音频 e2e**（ASR/TTS/扬声器，与本 change 正交）+ archive 待做。**⚠️ 更广隐患**：(c) 说明 ECS engine release（5/17）对「未被单独 scp 过」的文件可能整体滞后 main（scp-overwrite 部署只覆盖改动文件）→ **引擎全量对齐 main 应另起排查**。referee.py 回滚=scp 旧版（恢复 pass/hold primer，不建议）；provider/prompt 回滚见上。

Prior: 2026-06-11 11:28 CST — **remove-openai-provider deployed**（web + engine + api + common 全栈，**无 alembic / 无 DB**）。彻底删除 openai 作为可配置 LLM provider（前端 SSOT 选择器 + 「模型厂商」凭据卡 + engine factory 分支/默认值 + api `ALLOWED_PROVIDER_IDS` + common `ENV_KEY_MAP` + 2 spec + deploy 文档），并把 mock 从 campaign 角色 provider 下拉移除（mock **仍保留**在 engine factory `KNOWN_LLM_PROVIDERS` + api 白名单，供测试 + dev `ISALES_ENGINE_LLM_PROVIDER=mock` 模式）。**根因**：openai.com 从国内 ECS 不可达 + 生产 LLM 实为 dashscope（本条 smoke 实证 `credentials_loaded providers=['dashscope','volcengine']`，openai 从未在凭据集里）→ openai 是又死又用不上的能力；mock 摆进客户面下拉 = footgun（真活动误配成假 LLM）。顺带修 stale bug：dashscope 之前 UI 标「占位/engine 未对接」，实际 factory 早有分支 + 生产在用，纠正为已对接（`BACKEND_IMPLEMENTED += dashscope`）。**保留** `OpenAICompatibleLLMProvider` 共享类（OpenAI-**兼容协议**实现，volcengine/dashscope 共用，非 openai provider）。**部署**：web build（entry `index-BlidkFk7.js`→`index-CLskWhht.js`，含 `ModelProviderConfig-BumKAM7h.js` + `CampaignDetail-zi_yt2M3.js`）→ 备份 `/opt/isales/backups/web-remove-openai-provider-20260611-112531.tgz` → rsync `--delete` dist → `/var/www/isales-web` → nginx reload。scp editable 源码 common(`cli/cred_migrate.py` + `providers/_errors.py`)+engine(`providers/factory.py` + `settings.py` + `providers/llm_openai_compatible.py`)+api(`routers/provider_credentials.py`) → chown isales:isales + 清 pyc → **import smoke**（engine `KNOWN_LLM_PROVIDERS` + api `ALLOWED_PROVIDER_IDS` 均 `['dashscope','mock','volcengine']`、common `ENV_KEY_MAP` openai keys=`[]`、`build_llm("openai")`→`NotImplementedError`「not supported」）→ 重启 engine+api（scheduler/worker 不重启：不 import 改动文件）。**runtime no-op**（openai 从没被配/调用过，纯清死代码；唯一可见变化是 web 不再列 openai/mock）。**smoke**：engine `cloud_edge_grpc_server_started`+`credentials_loaded count=5 providers=['dashscope','volcengine']`+`isales_engine_started` clean、api `Application startup complete`（0.0.0.0:8000）、`/health`→200、`/provider-credentials`→401、nginx 公网 `/`→200、重启后 engine+api error/traceback=0、deployed web entry=`index-CLskWhht.js`。commit web `1e5981b` / api `a5ec948` / engine `a17b253` / common `faef51b` / meta `fc89501`(apply+archive)+本条 STATE。测试：web tsc clean + vitest 88、engine 404 passed、common 181、api 110（本 change 触及范围全绿；3 个 full-suite 红=pre-existing engine gate `test_gate_fails_open_to_main_on_referee_timeout` + api redis-steal by `_isales` 遗留守护进程，均 stash 实证无关）。**openspec `remove-openai-provider` 已 archive**（meta `fc89501`，2 MODIFIED delta 合入 web-admin-ui + provider-credential，validate --strict valid）。**回滚**：web rsync 回 `web-remove-openai-provider-20260611-112531.tgz` + nginx reload；scp 旧源码（恢复 engine openai 分支 + api `ALLOWED_PROVIDER_IDS`/common `ENV_KEY_MAP` openai 条目）+ 重启 engine+api（无 alembic 无 DB 回滚）。

Prior: 2026-06-11 10:42 CST — **per-role-llm-config-and-restructure-card deployed**（common + engine + api + web 全栈，**无 alembic**）。让每个 AI 角色（role_config）的 LLM provider/model 真正端到端生效 + 补 webui 缺失的「重组(restructure)」配置卡。**根因**：引擎为整进程建单一全局 LLM（`main.py` `build_llm(engine_llm_provider)`），所有 slot（main/persona/referee/restructure）的 PipelineStream 都复用它、`chat_stream` 只透传 temp/top_p、从不读 `spec.model`/`ext_params.provider` → 运营在角色卡选的 provider/model 一直是摆设；restructure 引擎已支持但前端无入口（唯一 `RoleConfigDialog` 在未挂载孤儿 `RoleConfigTab`）。**改法**：①common `_SlotSpec` 加 `provider` 字段（SSOT=`role_config.ext_params.provider`）+ `RestructureSpec.label` 改可选；②engine 新 `providers/llm_registry.py` `LLMRegistry`：`resolve(provider,model)` 惰性 `build_llm(...,model=)` + 按 `(provider,model)` 缓存 + **单层**全局回落（provider 空/缺凭据→WARN `llm_slot_provider_fallback`+用默认，不中断通话），`runtime_config._spec_for` 读 ext_params.provider 进 spec，`run_loop`/`orchestrator` 6 调用点 + `start()` per-referee 经 registry 解析 client，per-call `aclose_all`；③api `routing_validation` 去掉 `RESTRUCTURE` 的 label 必填；④web `CampaignDetail` 加 restructure singleton 卡（无 label / RefreshCw / gray）。transfer-LLM 仍用全局默认（Non-Goal）。extractor 在 worker+mock，ExtractorSpec 已带 provider，worker 接真 LLM 时再 honor（本期引擎不动）。**无 alembic**（provider 复用既有 ext_params JSONB，model 列已存在）。**部署**：scp common(pipeline.py)+engine(main/run_loop/runtime_config/orchestrator + 新 llm_registry.py)+api(routing_validation.py) editable 源码（current=releases/20260517-222944，engine+api 共用 venv，common editable 一处两端生效）→ ast-parse + import smoke（`LLMRegistry` import OK + `_SlotSpec.provider=True`）→ 重启 engine+api。web build（entry `index-BlidkFk7.js`，`CampaignDetail-BWfEagoV.js` 含重组卡）→ scp `dist/{assets,index.html}` → `/var/www/isales-web`（未 rsync --delete，旧 hashed assets 留存无害）。**smoke**：engine `cloud_edge_grpc_server_started`+`credentials_loaded count=5 providers=['dashscope','volcengine']`+`isales_engine_started` clean、api active、nginx `/`→200、index.html 引用新 bundle。commit common `0c48d5a`(0.8.8→0.8.9) / engine `87cc26f` / api `5523f7b` / web `cc516ba` / meta（本条提交）。测试：common 10(pipeline) / engine **405 passed**（+8 新 `test_llm_registry`；1 pre-existing gate `test_gate_fails_open_to_main_on_referee_timeout` 失败无关，stash 验证 clean tree 同红）/ api routing 20（含新 restructure-no-label；2 pre-existing redis-steal 无关，stash 验证）/ web **88** + vue-tsc clean。**⚠️ 行为变化**：per-slot model 从「被忽略」变「生效」——运营既有 campaign 角色的 provider/model 选择从此真生效；provider 未配的 slot 仍走全局默认（行为不变），只有显式配了非默认 provider 的 slot 才切换。**真机验收（tasks 5.2/5.3）待做**：配非默认 provider 的 slot 看 `llm_slot_provider_fallback`/resolution 日志 + 建 restructure 卡 + 路由规则 → barge-in 验重组真触发（需 mac 拨测）。**openspec `per-role-llm-config-and-restructure-card` 已 archive**（meta `78256b1`，3 ADDED delta 合入 specs/，validate 22 passed）。**HOTFIX api `07652f1`（真机保存逮到 422）**：`routers/role_configs.py` 有**第二份** `_LABELLED_KINDS`（`_REFEREE_RESTRUCTURE_KINDS`），standalone `POST /role-configs`（web 重组卡走这个、非 campaign 嵌套写）仍要求 restructure 带 label → `422 role_label_required`；去掉 RESTRUCTURE + 加 `test_restructure_without_label_accepted`，scp role_configs.py + restart api（`_REFEREE_KINDS` grep=1 + active）。**教训**：同一 label-kind 常量散在两处（`routing_validation.py` + `routers/role_configs.py`），改一处漏一处 = 重复定义 tech debt；正是 deferred 真机验收逮到的。**回滚**：scp 旧源码（engine 各文件 + `rm llm_registry.py`、common pipeline.py、api routing_validation.py + role_configs.py）+ 重启 engine+api + web 重 build 前一 commit 重新 scp（本次无 tgz 备份）+ nginx reload（无 alembic 无 DB 回滚）。
...
| ICP 备案 | **pending** — domain `tommax.cc` registered (2026-06-21), `isales`/`aidcp.tommax.cc` planned; **备案 NOT yet filed** → domains will not resolve-and-open on :80 until ICP done. Use IP `http://121.89.85.150/` meanwhile. | required if/when domain is registered |
| nginx | **active (2026-05-19, `web-admin-deploy`; server_name added 2026-06-21)** — nginx 1.20.1, single instance (master pid 720). Three :80 server blocks: (a) `isales.conf` `listen 80 default_server` `server_name isales.tommax.cc` → `isales-web` SPA `/var/www/isales-web/` + `/api/`→`127.0.0.1:8000` + `/ws/`→`127.0.0.1:8000/ws/`; (b) **co-tenant** `aidcp-console.conf` `listen 8088` + `listen 80` `server_name aidcp.tommax.cc` → `/opt/aidcp/console` + `:8090` (NOT isales-managed, not in this repo); (c) stock `nginx.conf` `:80 server_name _` (shadowed by default_server). engine gRPC still binds `0.0.0.0:50051` directly (un-fronted); isales-api still binds `0.0.0.0:8000` directly (Swagger fallback). | nginx terminates TLS + serves all SPA + reverse-proxies the four cloud services (full domain + 备案 path) |
