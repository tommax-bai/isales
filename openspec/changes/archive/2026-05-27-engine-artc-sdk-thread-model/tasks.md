...
- [x] 5.3 验证 sidecar log（`/tmp/Ali_RTC_Log_Client/aio_logger/<pid>_*/0_0.log`）含至少 1 条对 `gw.rtn.aliyuncs.com` 的 HTTP 请求；MUST NOT 全部指向 httpdns <!-- 部分达标：DNS 解析了 gw.rtn.aliyuncs.com → 60.205.93.126 (pre-fix 是 0)；但没有跑到 实际 HTTP 信令 — SDK 在 AliRTCEngine.cc:461 报 "Invalid token!" 当场 reject，没到 gw 信令阶段. **SUPERSEDED 2026-05-27 by `engine-rtc-dingrtc-migration`**: sidecar IPC 模型整体作废 (vendor 整个 sidecar TCP 路径不再存在); DingRTC 3.x in-process C++ 调用经 pybind11 binding, 没有 sidecar log 概念. dingrtc-migration § 5.4 ECS PCM loopback PASS (channel `rtc-smoke-1779785584` GSLB statusCode:200 + OnJoinChannelResult code=0 + 1001 inbound frames). 见 acceptance.md. -->
- [x] 5.4 验证 Python log（`/tmp/Ali_RTC_Log_Biz/Python-*.log`）含 `OnJoinChannelResult` 回调记录且 result code 为 0 <!-- 未达标：OnJoinChannelResult 从未 fire — 因 SDK 在本地 token 验证就拒了 (OnError code=16974594 fired instead); 但这恰好证明 callback marshal 链路正常 (OnError 从 driver 线程通过 call_soon_threadsafe 到 main loop 工作). **SUPERSEDED 2026-05-27 by `engine-rtc-dingrtc-migration § 5.4`**: 在 DingRTC binding 下 `OnJoinChannelResult(code=0, channel="rtc-smoke-1779785584", elapsed=128ms, streams_size=2)` 实证 fire 在 ECS, isales-engine@243fbc1. -->
- [x] 5.5 验证 smoke 进程在 10s 时长内收到回环 PCM 帧（stdout 看回环字节数 > 0） <!-- 未达标：join 失败连接没建立，没有 PCM 流. **SUPERSEDED 2026-05-27 by `engine-rtc-dingrtc-migration § 5.4`**: ECS smoke 10s 回环输出 `inbound_frames=1001, inbound_bytes=1921920, first_inbound_frame_at_s=0.00064`, exit 0 (isales-engine@243fbc1). dingrtc-migration § 8.8 还跑通 mac+ECS 双 peer 同房间 inbound 各 ≥1900 帧 (channel `dual-peer-1779788945`). -->
- [x] 5.6 5.3 / 5.4 / 5.5 任一失败 → 不视为完成；回到 §2 调 pump 间隔 / dispatch 时序；MUST NOT archive <!-- 失败原因 NOT 在 pump interval / dispatch 时序 — 是 token mint 层. **SUPERSEDED 2026-05-27**: spec gate 的前提"调 pump/dispatch 时序"已不适用; root cause 是 ApsaraVideo Live ARTC SDK 与真 AppId 跨产品线 (DingRTC PaaS); dingrtc-migration 换 SDK 整体绕开. archive 解锁条件 = SUPERSEDED by dingrtc-migration 而非"token 修复 + 复跑 smoke". -->
...
- [x] 6.6 准备 archive：等 A2 e2e 通话验收完整跑过 1 次（本变更是 join 层 enabler，不强求 archive 时已跑完 e2e 通话；视 A2 整体节奏）→ `/opsx:archive engine-artc-sdk-thread-model` <!-- 2026-05-27 archived as SUPERSEDED by `engine-rtc-dingrtc-migration` (driven from migration § 11.1). 不是按"token 修复 + 复跑 smoke"路径解锁, 而是 vendor SDK 换产品线 (ApsaraVideo Live ARTC → DingRTC PaaS), sidecar IPC 模型整体作废. dingrtc-migration § 6.2/6.3 删除了本 change 改动的 `transport/aliyun_rtc.py` + `_rtc_sdk.py`; § 6.4 显式记录 `_rtc_driver.py` (sidecar pump 1410 行) 只在本 change branch 存在, 随 archive 一并作废. dingrtc-migration § 5.4 ECS smoke 全绿 (10s + 1001 inbound frames + clean exit) 是真正落地的 join-layer enabler. 见同目录 acceptance.md. -->
