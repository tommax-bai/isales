...
- [x] 5.1 Deployed full-stack: scp common+api editable (symlink-resolved, scp pass-through verified inode 654239), `pg_dump -t campaign` backup, `alembic upgrade head` (a8b9c0d1e2f3→b9c0d1e2f3a4, 0 do_not_call cols + FK gone), restart api/engine/scheduler/worker, web build + backup `.bak-20260612-151935` + rsync --delete + nginx reload. Smoke: scheduler campaign SELECT works on new schema (decisive), `:8000/docs`→200, `/campaigns`→401, public SPA→200, openapi do_not_call=1 (kept LeadStatus enum; campaign schemas=0), served DoNotCall component refs=0. STATE.md updated (head b9c0d1e2f3a4, common 0.8.14). <!-- deploy 2026-06-12 15:20 CST -->
- [x] 5.2 Doc sync: grep of `README.md`/`DESIGN.md`/`IMPLEMENTATION_PLAN.md`/`CLAUDE.md` for do_not_call config mentions → none (data-model column list is delta-merged via the spec; no root-doc reconciliation needed).
- [x] 5.3 Archived `remove-do-not-call-campaign-config` (verified dir moved to `changes/archive/`). Hand-stripped the 3 `campaign.do_not_call_*` rows from `openspec/specs/retry-followup/spec.md`'s free-form `## Data Schema` table (requirement-only delta engine does not touch that section).
