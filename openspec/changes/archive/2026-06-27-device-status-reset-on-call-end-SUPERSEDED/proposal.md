...
>
> 本提案的事件驱动重设计（`DeviceReleased` 消息 + `device_reset` 队列 + scheduler 消费者 +
> 心跳改 liveness-only）**从未实装**（origin/main 全仓零引用）。期间问题已从两侧解掉：
> ①关联的 bug2 真根因不是"没人写回 device"，而是引擎 `EngineHandle::destroy_unlocked` 持 GIL
> 与 DingRTC SDK 事件线程 listener 回调死锁致引擎冻死——已由 binding 0.1.0→0.1.1 修复
> （engine `753ff13`，见 deploy/cloud/STATE.md:1135，非本 change）；②device→idle 复位由现有
> **心跳路径**承担（edge `_status_provider` 报 IN_CALL/IDLE → engine `heartbeat_handler` 写
> `device.status`）——代码库选择**保留并修好**了本提案明确要删的那条心跳载体，方向与提案相反。
> 撤回此提案。若仍需 engine-crash 兜底（引擎在 finalize 前崩溃、心跳路径无法复位的独立失败域），
> 单独窄范围重提 watchdog 一项即可（dialing/in_call + stale last_call_at → idle）。
