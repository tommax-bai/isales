...
- [x] 2.6 `main.py` `_on_manual_hangup`/`_on_transfer`:改 `sess.bus.post(ManualHangupRequested/TransferRequested)`;删 `_ManualHangupRequested` 异常类 + 死 `except` 块 + `request_manual_hangup`。
  ...
  - **控制 bridge**:`_subscribe_control_bridges(session, bus)` 订阅 `ManualHangupRequested`/`TransferRequested` → `_terminate(cause)`(= 旧 `request_manual_hangup` 体:设 `MANUAL_HANGUP` + `main.cancel()`),**逐字节等价**(含 transfer→MANUAL_HANGUP stage-4 stub)。死 `except _ManualHangupRequested` 确认从不触发(裸 CancelledError),删除无行为影响;manual finalize 一直在 dial_consumer 侧,没动。
  - **review must-fix 已修(pre-dial 窗口)**:bus 起初放 run_session 入口建,但那在 runner 的 DB 前置(查 Lead + load_runtime_config,两次 DB 往返)之后,而 session 在拨号 spawn 前就注册 + 设 `tasks["main"]`——此窗口的 manual hangup 会命中 `sess.bus is None` 被**丢**(旧 `request_manual_hangup` 不会丢),是 2.6 引入的唯一非-byte-identical 偏差。修法:bus 改 **与 CallSession 同寿构造**,窗口内事件 post 即 buffer 到无损道(post-before-start),start 后 dispatcher drain 处理,**一个不丢**,恢复 byte-identical。commit `3950d28`。
  - **HAZARD 解除**:bus 归 run_session.finally 托管后,1a 留的 change-2 dispatcher 泄漏隐患自动消解。
