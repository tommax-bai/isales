...
- [~] 5.1 切真 AppId `o6dpsan9...` + 真 AppKey（从 `provider_credential` 表 Fernet 解密装载）跑 `dingrtc_real_smoke.py` → `OnJoinChannelResult(code=0)` <!-- 2026-05-26 ECS 跑 `dingrtc_demo_smoke.py --real`. SDK 启动 + 联网 + JoinChannel 信令实际发出（log "[API] JoinChannel (appid=o6dpsan9, channel=ch1, userid=u1, gslb=https://gslb.dingrtc.com, nick=u1)"）—— 跨过 token 本地解析（无 0x01030302）+ 跨过 token roomserver 验证（无 0x02010205）。**新 blocker**：`RtcEngineErrorJoinChannelFailed 0x01030202`, message "gslb returned error: -1(invalid json body)". 排查结果: (A) AppId 确认完整 = `o6dpsan9` 8 字符（用户口述 2026-05-26）→ 不是截断问题；(B) `gitee.com/dingrtc/AliRTCSample` 当前 404，memory "rtc_token.py v3.0 binary 算法 100% 实证 = DingRTC 3.x" 的依据已失效。SDK 内部 GSLB request body 字段 (appId/channelId/userId/timestamp/requestId/deviceId 都是 camelCase JSON) 与我们 token v3.0 binary 算法关系不明。**收敛假设**: 我们的 rtc_token.py v3.0 binary 实际仍是 AliRTC 2.x sample 形态, DingRTC 3.x roomserver/gslb 期望的 token 格式可能差异. 缺乏新 vendor sample 仓库无法验证. 后续需要用户提供 (1) DingRTC 控制台核对 AppId `o6dpsan9` 是否在 DingRTC 3.x 产品下激活（也可能它只在 AliRTC 2.x 控制台存在）, (2) 新 vendor token sample 链接 (现行 vendor docs / DingRTC console "服务端开发指南" tab 下载链接), (3) 或开 ticket 拿 sample. -->
