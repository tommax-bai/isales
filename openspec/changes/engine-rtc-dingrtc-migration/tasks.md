...
- [ ] 1.1 在 commit ccb10dd 打 git tag `pre-dingrtc-migration` 作整体回退基线（meta-repo + 7 sub-repo）—— 建议等 §2 实质 binding 第一个 commit 前再打更有意义
- [ ] 1.2 **[user action]** 下 DingRTC Linux SDK 3.9.0 到 `/opt/isales/vendor/DingRTC_Linux_SDK_3_9_0/`（cloud ECS + 本机 dev 都装）；校验 sha256 写进 `deploy/cloud/STATE.md` § "DingRTC SDK vendor"
- [ ] 1.3 **[user action]** 下 DingRTC Windows SDK 3.9.0 到 `~/codes/vendor/DingRTC_Windows_SDK_3_9_0/`（Windows edge 机）；校验 sha256 写进 `isales-telephony/deploy/edge/windows/STATE.md`
- [ ] 1.4 **[user action]** 下 DingRTC macOS SDK 3.9.0 到 `~/codes/vendor/DingRTC_macOS_SDK_3_9_0/`（dev mac + 本机）；校验 sha256 写进 `isales-telephony/deploy/edge/macos/RUNBOOK.md`
...
- [ ] 1.7 在 `isales-engine` 起 worktree `dingrtc-migration-cloud`（base = `main`，**不** base 在当前 `engine-artc-sdk-thread-model` worktree）；在 `isales-telephony` 起 worktree `dingrtc-migration-windows` + `dingrtc-migration-macos`（都 base = `main`）
- [ ] 1.8 **[blocked on 1.4]** `class-dump ~/codes/vendor/DingRTC_macOS_SDK_3_9_0/DingRTC.framework/DingRTC > /tmp/dingrtc_macos_selectors.txt` 提取 Obj-C selector 名；记录到 `openspec/changes/engine-rtc-dingrtc-migration/notes/macos_selectors.md`（新建）

## 2. Cloud Linux engine：pybind11 binding 最小集

- [ ] 2.1 `isales-engine/isales_engine/transport/dingrtc/` 目录骨架：`CMakeLists.txt` + `binding.cpp` + `audio_observer.h/.cpp` + `engine_listener.h/.cpp` + `__init__.py`（参考 `windows-artc-pybind11` 通用层结构）
- [ ] 2.2 pyproject 加 scikit-build 或等价 build backend；`pip install -e .` 触发 binding 编译；产 `dingrtc_pywrap.so` 到 `transport/dingrtc/`
- [ ] 2.3 binding 暴露 `RtcEngine::Create` / `Destroy` / `SetEngineEventListener` / `JoinChannel(authInfo, userName)` / `LeaveChannel` 五个 API
- [ ] 2.4 binding 暴露 `SetExternalAudioSource(true, sampleRate, channels)` + `PushExternalAudioFrame(RtcEngineAudioFrame)` 两个 API
- [ ] 2.5 binding 实施 `RtcEngineAudioFrameObserver::OnPlaybackAudioFrame(frame)` callback → 通过 `main_loop.call_soon_threadsafe(...)` marshal 到 Python async iterator
- [ ] 2.6 binding 实施 `RtcEngineEventListener::OnJoinChannelResult` / `OnLeaveChannelResult` callback → marshal 到 Python `asyncio.Future`
- [ ] 2.7 vendor 头文件路径 / 共享库路径通过环境变量 `ISALES_DINGRTC_LINUX_SDK_PATH` override（CMakeLists 读 env）

## 3. Cloud Linux engine：Python wrapper + RtcSession ABC

- [ ] 3.1 新建 `isales-engine/isales_engine/transport/dingrtc.py`（或 `dingrtc/_session.py`），定义 `DingRtcSession(RtcSession)` 实施 `isales_common.audio.rtc.RtcSession` ABC 的 4 个 abstract API；命名 mirror 现有 `AliyunRtcSession`（**不**叫 `DingRtcChannel` —— 那是内部 SdkChannel 命名）
- [ ] 3.2 `join(channel, token, uid, *, send_sample_rate=16000, send_channels=1)`：组 `RtcEngineAuthInfo{channelId=channel, userId=uid, appId=<creds>, token=<token>, gslbServer="https://gslb.dingrtc.com"}` → 调 binding `JoinChannel` → 等 `OnJoinChannelResult` Future（10s 超时）
- [ ] 3.3 `leave()`：调 binding `LeaveChannel` + `SetEngineEventListener(nullptr)` + `Destroy(engine)`；idempotent
- [ ] 3.4 `is_joined` property + `audio_frames()` AsyncIterator + `push_audio(pcm, *, timestamp_ms)`（成功 / `RtcError` / `RtcPushBackpressure` 三种返回路径）
- [ ] 3.5 提供 `DingRtcSession.production(app_id=...)` classmethod 镜像现有 `AliyunRtcSession.production(...)`；从 `CredentialStore` 取 DingRTC AppId / AppKey（沿用既有 provider-credential 装载路径，字段名兼容）
- [ ] 3.6 `transport/dingrtc/__init__.py` export `DingRtcSession`（如 §3.1 选 dir 形态）；如选 module 形态则跳过
- [ ] 3.7 **[作废 / audit-only]** 原任务"重构 `audio_pipe/AliyunRTCCapture` / `AliyunRTCPlayback` 改名" —— 2026-05-26 ground-truth 验证（`git grep AliyunRTC` 在 `isales-engine`）：`audio_pipe/` 目录不存在；`AliyunRTC*` capture/playback backend 类不存在；唯一对外 RTC API 就是 `AliyunRtcSession`（已由 §3.1 + §6.1 覆盖重命名）。本任务作废，留 audit trail

## 4. Cloud Linux engine：PoC demo appid 隔离验证（必经）

- [ ] 4.1 写 `isales-engine/scripts/dingrtc_demo_smoke.py`：用 demo appid `a4zfr1hn` + demo AppKey + `rtc_token.RtcToken3.create_token` → `DingRtcSession.production(app_id=...).join("dingrtc-demo-<ts>", token, "smoke-uid-01")` → 等 `OnJoinChannelResult(code=0)`
...
- [ ] 5.1 切真 AppId `o6dpsan9...` + 真 AppKey（从 `provider_credential` 表 Fernet 解密装载）跑 `dingrtc_real_smoke.py` → `OnJoinChannelResult(code=0)`
...
- [ ] 6.1 所有 import `isales_engine.transport.aliyun_rtc.AliyunRtcSession` / `InMemorySdkChannel` / `SdkLoadError` / `isales_engine.transport._rtc_sdk` 改 import `isales_engine.transport.dingrtc.DingRtcSession` / 等价 mock（`git grep` 找全 —— 已知点：`isales_engine/main.py:49,91,96`, `scripts/ecs_pcm_loopback_listen.py:31,55`, `tests/test_aliyun_rtc_*.py`, `tests/test_artc_driver_thread.py`, `tests/test_rtc_telephony.py:388`）
- [ ] 6.2 删 `isales-engine/isales_engine/transport/aliyun_rtc.py`
- [ ] 6.3 删 `isales-engine/isales_engine/transport/_rtc_sdk.py`
- [ ] 6.4 删 `isales-engine/isales_engine/transport/_rtc_driver.py`（commit fb62fdd 1410 行 sidecar pump）
- [ ] 6.5 删 `isales-engine/tests/transport/test_aliyun_rtc*.py` / `test__rtc_sdk*.py` / `test__rtc_driver*.py` 全部旧 ARTC 测试
- [ ] 6.6 新增 `isales-engine/tests/transport/test_dingrtc_channel.py`：vendor binding mock + Future 路径 + `RtcError` / `RtcPushBackpressure` 失败路径 + leave idempotent
- [ ] 6.7 `pytest -q tests/transport/` 全套绿
...
- [ ] 9.1 `deploy/cloud/STATE.md` § "ARTC SDK vendor" → "DingRTC SDK vendor"：CDN URL / 解压路径 / 版本号 / sha256 全换；记录"切换日 + 实证 commit"
...
- [ ] 9.5 `isales-engine/CLAUDE.md`（如有）/ `isales-engine/README.md`：`transport/` 目录变更说明；新增 "DingRTC 3.x 集成" 段
