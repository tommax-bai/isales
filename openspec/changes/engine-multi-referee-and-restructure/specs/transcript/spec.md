<!-- base spec 已含 § "pipeline_trace 表的字段约束"（main + 单 referee 双 LLM trace）。
     本 delta 用 MODIFIED 把单 referee 字段改为 referee_results 数组 + 新增 restructure 记录字段，
     其余字段（main_* / first_audio_ms / error）与 UI 约束保留。
     与 data-model delta（pipeline_trace 列定义）同步。
