...
- [x] 1.7 alembic migration（一支）：DDL（label 列 / routing_rules 列 / restructure 相关 campaign 列 / pipeline_trace 字段重构 / 放宽 referee 唯一约束、给 referee label 加同-campaign unique index）<!-- a1b2c3d4e5f6 (down=f6a7b8c9d0e1)；referee「恰好一行」是 app 层约束非 DB，故 DB 侧只加 partial unique index uq_role_config_campaign_label WHERE label IS NOT NULL -->
