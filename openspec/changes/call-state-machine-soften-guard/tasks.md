...
- [ ] 4.5 ssh ECS 拉 `journalctl -u isales-engine | grep state_transition_unusual` 确认 warning log 可见 + 可 grep <!-- pending: 等 4.4 smoke 产生流量后 grep；grep 基础设施 ready (count=0 pre-smoke 已验) -->
- [ ] 4.6 SQL 查 call_record transcript 含 `state_warning` 事件结构正确（type / attempted / from_state / to_state 4 字段齐） <!-- pending: 等 4.4 smoke 产生通话记录 -->

> **当前 session 收尾 (2026-06-02)**: tasks 1.*-3.* + 4.1-4.3 全部 done，engine 已部署 ECS active。4.4 (mac mic smoke) / 4.5 (journalctl grep) / 4.6 (SQL transcript) / 5.* (Windows 真拨号) 留给下一次有物理硬件的 session。所有代码 + spec 改动已落 isales-engine `2cb47bc` + meta-repo tasks 回写。
