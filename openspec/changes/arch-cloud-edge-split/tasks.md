...
- [ ] 9.5 isales-api / isales-telephony / isales-scheduler / isales-worker：把 `isales-common` 依赖 pin 从 `>=0.1.x,<0.2` bump 到 `>=0.2.0,<0.3`，跟 isales-engine（task 9.x 实施期间已用 0.2 接口）对齐  <!-- task list 漏项：9.1/9.2/9.3/9.4 的代码事实上已经在 import isales_common.proto / cloud_edge transport ABC（v0.2.0 新增），但 4 个下游仓 pyproject pin 还停在 0.1.x，导致 `pip install -e ".[dev]"` 在本地 sibling 模式下解析失败。每仓独立 commit + push。 -->
