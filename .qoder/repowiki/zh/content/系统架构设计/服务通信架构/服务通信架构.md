...
- [双LLM管线与旁路提取器](file://openspec/changes/pipeline-stream-and-referee/specs/service-communication/spec.md)
- [双LLM管线与旁路提取器提案](file://openspec/changes/pipeline-stream-and-referee/proposal.md)
- [双LLM管线与旁路提取器设计](file://openspec/changes/pipeline-stream-and-referee/design.md)
- [AI管线规范](file://openspec/changes/pipeline-stream-and-referee/specs/ai-pipeline/spec.md)
- [数据模型规范](file://openspec/changes/pipeline-stream-and-referee/specs/data-model/spec.md)
...
## 更新摘要
**所做更改**
- 新增 isales:extract Redis队列条目的详细说明
- 定义post-call extractor通信协议和消息契约
- 更新服务通信通道矩阵，新增engine→worker的extract队列
- 完善异步抽取器的容错和重试机制
- 增强数据模型中extract_status字段的语义说明
...
**更新重点**：本次更新重点关注新增的isales:extract Redis队列条目，定义post-call extractor通信协议，实现通话结束后的异步客户信息抽取功能。
...
- **新增**：isales:extract队列：通话结束后异步抽取客户信息的专用队列。
...
- [双LLM管线与旁路提取器:5-21](file://openspec/changes/pipeline-stream-and-referee/specs/service-communication/spec.md#L5-L21)
...
### Redis 在服务通信中的多重作用
- 队列（Queue）：必须送达的消息派发，如scheduler→engine拨号请求、engine→worker通话结束处理、**engine→worker异步抽取任务**、api→scheduler启动/暂停Campaign。
...
### 异步抽取器通信协议（新增）
**更新**：新增isales:extract Redis队列条目，定义post-call extractor通信协议

- **队列名称**：`isales:extract`
- **触发时机**：engine在END状态前LPUSH任务到队列
- **消息格式**：JSON字符串，包含call_record_id、transcript_snapshot、extractor_role_config_id、extractor_prompt_version_id
- **处理流程**：worker服务的post_call_extractor consumer BLPOP队列，调用LLM provider结构化抽取，成功后UPDATE call_record.extracted字段
- **状态管理**：engine LPUSH时UPDATE extract_status='pending'，worker处理完成后UPDATE为'done'或'failed'
...
participant DB as "PostgreSQL"
ENG->>ENG : "END状态前准备抽取任务"
ENG->>REDIS : "LPUSH isales : extract<br/>{call_record_id, transcript_snapshot, extractor_role_config_id, extractor_prompt_version_id}"
ENG->>DB : "UPDATE call_record.extract_status='pending'"
Note over ENG,DB : "engine不等待结果，继续通话流程"
WORKER->>REDIS : "BLPOP isales : extract"
REDIS-->>WORKER : "返回抽取任务"
WORKER->>LLM : "chat(json_mode=True)<br/>输入 : 完整transcript_snapshot + extractor prompt"
LLM-->>WORKER : "输出 : {extracted : {...}}"
WORKER->>DB : "UPDATE call_record SET extracted=<result>, extract_status='done'"
Note over WORKER,DB : "失败时UPDATE extract_status='failed' + extract_error=<reason>"
```

**图表来源**
- [双LLM管线与旁路提取器:28-48](file://openspec/changes/pipeline-stream-and-referee/specs/service-communication/spec.md#L28-L48)
- [AI管线规范:130-152](file://openspec/changes/pipeline-stream-and-referee/specs/ai-pipeline/spec.md#L130-L152)
- [设计:109-131](file://openspec/changes/pipeline-stream-and-referee/design.md#L109-L131)

**章节来源**
- [双LLM管线与旁路提取器:28-48](file://openspec/changes/pipeline-stream-and-referee/specs/service-communication/spec.md#L28-L48)
- [AI管线规范:130-152](file://openspec/changes/pipeline-stream-and-referee/specs/ai-pipeline/spec.md#L130-L152)
- [设计:109-131](file://openspec/changes/pipeline-stream-and-referee/design.md#L109-L131)
- [数据模型规范:59-67](file://openspec/changes/pipeline-stream-and-referee/specs/data-model/spec.md#L59-L67)
...
- **新增**：异步抽取器容错：worker BLPOP后处理失败不重LPUSH（防雪崩），UPDATE extract_status='failed' + extract_error=<reason>，ops通过SQL查询WHERE extract_status='failed'手工触发重跑。
...
Exhausted --> End
Wait --> End
Start2(["抽取器失败"]) --> Err{"错误类型？"}
Err --> |LLM超时/JSON校验失败/DB写失败| NoRequeue["不重LPUSH(防雪崩)"]
NoRequeue --> Update["UPDATE extract_status='failed'<br/>+ extract_error=<reason>"]
Update --> Ops["ops通过SQL查询<br/>WHERE extract_status='failed'<br/>手工触发重跑"]
Ops --> End2(["完成"])
```
...
- [双LLM管线与旁路提取器:44-48](file://openspec/changes/pipeline-stream-and-referee/specs/service-communication/spec.md#L44-L48)
...
- **新增**：异步抽取器依赖关系：engine通过isales:extract队列与worker解耦，worker独立处理抽取任务，支持水平扩展。
...
- **新增**：异步抽取器性能优化：engine不等待抽取结果，worker可水平扩展处理大量并发抽取任务，避免阻塞通话主流程。
...
- [引擎实现设计:168-178](file://openspec/changes/archive/2026-05-07-impl-engine/design.md#L168-L178)
- [云边gRPC保活:1-57](file://openspec/changes/cloud-edge-grpc-keepalive/specs/service-communication/spec.md#L1-L57)
- [双LLM管线与旁路提取器:109-131](file://openspec/changes/pipeline-stream-and-referee/design.md#L109-L131)
...
- **新增**：异步抽取器故障排查：检查extract_status字段状态，WHERE extract_status='failed'查询失败任务，手工重跑LPUSH命令；监控worker进程水平扩展情况。
...
**更新总结**：本次更新完善了异步抽取器通信协议，通过isales:extract队列实现engine与worker的完全解耦，提升系统整体性能和可扩展性。新增的extract_status字段提供了完整的抽取状态跟踪，配合容错机制确保系统的稳定运行。

## 附录
- 环境变量示例：API/JWT/Redis/数据库等关键配置参考部署环境示例文件。
- 变更流程：所有协议与契约变更需通过OpenSpec变更提案流程，确保可追溯与一致性。
- **新增**：异步抽取器运维指南：提供extract_status字段查询方法、失败任务重跑流程、worker服务水平扩展配置。

**章节来源**
- [API环境示例:1-14](file://deploy/env/api.env.example#L1-L14)
- [云环境示例:1-24](file://deploy/cloud/env/api.env.example#L1-L24)
- [双LLM管线与旁路提取器:160](file://openspec/changes/pipeline-stream-and-referee/tasks.md#L160)